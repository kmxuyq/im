(function($) {
	...
	function update_screen_focus(){
	    ...
	    if (ap_ids != '') {
    		$.ajax({
    			type: "GET",
    			url: WAP_SHOP_SITE_URL+'/index.php?act=adv&op=get_adv_list'+ap_ids,
    			dataType:"jsonp",
    			async: true,
...
$(function(){
	...
	/*监听滚动条和左侧菜单点击事件 start b y 33 hao.com*/
            ...
            window.onscroll = function(){
                ...
                if(_arr.length){
                    $('.nav_Sidebar a').eq(_arr[_arr.length-1]).css({'backgroundImage':'url('+WAP_SHOP_SITE_URL+'/templates/default/images/home-nav-icon-hover.png)'}).addClass('current').siblings().css({'backgroundImage':'url('+WAP_SHOP_SITE_URL+'/templates/default/images/home-nav-icon.png)'}).removeClass('current');
                    _arr = [];
                }
            }
            $('.nav_Sidebar a').each(function(index){
                $(this).click(function(){
                    ...
                    if($(this).hasClass('current')){
                        ...
                        $(this).css({'backgroundImage':'url('+WAP_SHOP_SITE_URL+'/templates/default/images/home-nav-icon-hover2.png)'});
                    }
                }).mouseout(function(){
                    if($(this).hasClass('current')){
                        return;
                    }else{
                        $(this).css({'backgroundImage':'url('+WAP_SHOP_SITE_URL+'/templates/default/images/home-nav-icon.png)'});
                    }
                });
            });
