...
//活动规则的展示隐藏，并相应的赋值
$(function(){
	$('input:radio[name="is_check"]').change(function(){
        if ($('#is_check_1').prop("checked")) {
            $('[nctype="virtual_valid"]').show();
            //改变值
            $('input[name="is_check"]').val('1');
        } else {
        	$('[nctype="virtual_valid"]').hide();
        	$("input:radio[name='is_collect']").eq(1).attr("checked",'checked');
        	$("input:radio[name='is_buy']").eq(1).attr("checked",'checked');
        	//改变值
            $('input[name="is_check"]').val('0');
            $('input[name="is_collect"]').val('0');
            $('input[name="is_buy"]').val('0');
        }
    });
	$('input:radio[name="is_collect"]').change(function(){
		if ($('#is_collect_1').prop("checked")) {
			 $('input[name="is_collect"]').val('1');
		}else{
			$('input[name="is_collect"]').val('0');
		}
    });
	$('input:radio[name="is_buy"]').change(function(){
		if ($('#is_buy_1').prop("checked")) {
