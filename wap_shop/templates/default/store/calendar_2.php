<?php  if($output['goods']['calendar_type'] ==2){ //酒店价格日历?>
    ...
    <script>
        ...
        $(".detail_check_in_time").click(function(){
            var url = "index.php?act=goods_stock_price&op=hotel_calendar&commonid=<?php echo $output['goods']['goods_commonid'];?>&is_cx=1&xs_id=<?php echo $_GET['group_id'];?>";
            var data = data || {};
...
<div class="goods_types <?php if(!empty($output['goods']['spec_name'])) echo 'border_top'?>"> <?php if($output['goods']['goods_state'] != 10 && $output['goods']['goods_verify'] == 1){?>
    ...
    <?php if($output['goods']['calendar_type'] ==1){//普通价格日历
        $stock_url = urlWapShop('goods_stock_price', 'index', array('commonid' => $output['goods']['goods_commonid'],'calendar_type'=>1, 'is_cx' => 1));
        ?>

        <div  id="hotel_ceshi" style="clear: both; overflow: hidden" style="font-size: 14px">
            <span title="<?php echo $stock_url;?>" id="rili_span_stock">
                <div href="javascript:void(0)" style="display: block">
            ...
            <input type="hidden" value="-1" name="spec_name" id="spec_name_input"   />
            <div class="bill_btns" style="line-height: 30px;display: none" ><input   type="hidden"  class="putong_man_input"  value="-1" />成人票：<i>￥</i><span id="putong_man_price"></span ></div>
            <div class="bill_btns" style="line-height: 30px;display: none" ><input   type="hidden"  class="putong_child_input"  value="-1" />儿童票：<i>￥</i><span id="putong_child_price"></span ></div>
            <div class="bill_btns" style="line-height: 30px;display: none" ><input   type="hidden"  class="putong_diff_price"  value="-1" />单房差：<i>￥</i><span id="putong_diff_price"></span ></div>
        </div>
    ...
    <input type="button" style="background-color: white;" id="picktime" value="" />
        ...
        <div class="buy_quantity">
            ...
            <div style="padding: 3px 0; clear: both;    font-size: 16px;line-height: 40px; height: 34px;" >
                ...
                <input type="hidden"  id="calendar_hour_input" value="" >
            </div>
            <div style="padding: 3px 0; clear: both;    font-size: 16px;line-height: 40px;height: 34px;" >
                <span>预订时段(分钟)</span>
                ...
                <input type="hidden"  id="calendar_min_input" value="" >
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        $('#rili_span_stock').click(function(){
            ...
            <?php if($output['goods']['calendar_type'] == 1) : ?>
                var common_id= <?php echo intval($output['goods']['goods_commonid']);?>;
                var package = $("#spec_name_input").val();
                /*如果添加了商品规格及价格日历套餐 begin */
                <?php if(!empty($output['goods']['spec_name'])): ?>
                    ...
                    url = '?act=goods_stock_price&op=index&commonid=' + common_id + '&calendar_type=1&is_cx=1&package=' + package;
    ...
    <?php if($output['goods']['calendar_type'] ==2 || $output['goods']['calendar_type'] ==3){ ?>
        <!--   酒店和高尔夫单个存订单，数量为1-->
        <div class="buy_choice" style="display:none;">
            <div class="kucun">
    ...
    <?php }else if($output['goods']['calendar_type'] == 4){ ?>
        ...
        <div class="buy_choice">
            <?php if(isset($output['goods']['get_date'])){ ?>
                ...
                <div class="kucun" style="display:none;">
                    <span>销售： <?php echo $output['goods']['goods_salenum'];?></span>
                    <span style="margin-left: 10px">库存：<?php echo $output['goods']['storage'];?></span>
                </div>
                <div class="mykucun" style="display:none;">库存： <span><?php echo $output['goods']['storage']; ?></span></div>
            <?php }else{ ?>
                <div id="station_ticked_box" style="display:none">
                    ...
                    <div class="mykucun" style="display:none;" ><span id="station_ticked_storage_h"></span></div>
                </div>
    ...
    <?php }else{ ?>
        <div class="buy_choice">
            ...
            <div class="buy_quantity">
                <div class="title" style="text-align: left;">购买数量</div>
                <div class="choice_num">
                    <a href="javascript:;" class="min"></a>
                    <input type="text" class="num" id="quantity" value="1" onkeyup="this.value=this.value.replace(/[^\d]/g,'1') " onafterpaste="this.value=this.value.replace(/[^\d]/g,'1')" />
                    <a href="javascript:;" class="plus"></a><span id="stock_msg"></span>
            ...
            <?php if($_SESSION['share_shop'] == 1 and $output['goods']['isshare']==1): ?>
                <div class="mykucun" style="display:none;">库存： <span><?php echo $output['goods']['share_stock']; ?></span></div><!--分销库存 -->
            <?php else:?>
                <div class="mykucun" style="display:none;">库存： <span><?php echo $output['goods']['goods_storage']; ?></span></div>
            <?php endif;?>
        </div>
