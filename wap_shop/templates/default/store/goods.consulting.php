...
<div class="top" style="overflow: hidden;">
  ...
  <div class="ncs-cosult-askbtn"><a href="<?php echo urlShopWAP('goods', 'consulting_list', array('goods_id' => $_GET['goods_id']));?>#askQuestion" target="_blank" class="ncs-btn ncs-btn-red">我要提问</a></div>
</div>
<div class="ncs-goods-title-nav">
  <ul id="consulting_tab">
    <li class="<?php if (intval($_GET['ctid']) == 0) {?>current<?php }?>"><a href="<?php echo urlShopWAP('goods', 'consulting', array('goods_id' => $_GET['goods_id'], 'store_id' => $_GET['store_id']));?>">全部</a></li>
    <?php if (!empty($output['consult_type'])) {?>
    <?php foreach ($output['consult_type'] as $val) {?>
    <li class="<?php if (intval($_GET['ctid']) == $val['ct_id']) {?>current<?php }?>"><a href="<?php echo urlShopWAP('goods', 'consulting', array('goods_id' => $_GET['goods_id'], 'ctid' => $val['ct_id'], 'store_id' => $_GET['store_id']));?>"><?php echo $val['ct_name'];?></a></li>
    <?php }?>
    <?php }?>
  </ul>
</div>
<div class="ncs-cosult-main">
  ...
  <?php }?><div class="more"><a href="<?php echo urlShopWAP('goods', 'consulting_list', array('goods_id' => $_GET['goods_id']));?>" target="_blank" >查看全部咨询>></a></div>
  <?php } else { ?>
