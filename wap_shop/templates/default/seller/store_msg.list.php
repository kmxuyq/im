...
<table class="ncsc-default-table">
  <thead>
    ...
    <tr>
      ...
      <td colspan="20"><label for="all">全选</label>
        <a href="javascript:void(0);" nc_type="batchbutton" uri="<?php echo urlShopWAP('store_msg', 'mark_as_read');?>" name="smids" class="ncsc-btn-mini"><i class="icon-flag"></i>标记为已读</a>
        <?php if ($_SESSION['seller_is_admin']) {?>
        <a href="javascript:void(0);" nc_type="batchbutton" uri="<?php echo urlShopWAP('store_msg', 'del_msg')?>" name="smids" class="ncsc-btn-mini"><i class="icon-trash"></i>删除</a>
        <?php }?>
      </td>
    </tr>
  </thead>
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle"><span><a href="javascript:void(0);" class="btn-blue" dialog_id="store_msg_info" dialog_title="系统消息" dialog_width="480" nc_type="dialog" uri="<?php echo urlShopWAP('store_msg', 'msg_info', array('sm_id' => $val['sm_id']));?>"><i class="icon-eye-open"></i>
        <p>查看</p>
        </a></span></td>
    </tr>
    <?php } ?>
