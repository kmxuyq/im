...
<?php if ($output['isOwnShop']) { ?>
  <a class="ncsc-btn ncsc-btn-green" href="<?php echo urlShopWAP('store_promotion_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a>

<?php } else { ?>

  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <a class="ncsc-btn ncsc-btn-green" style="right:100px" href="<?php echo urlShopWAP('store_promotion_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a> <a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo urlShopWAP('store_promotion_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo urlShopWAP('store_promotion_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i><?php echo $lang['xianshi_quota_add'];?></a>
  <?php } ?>
    ...
    $(document).ready(function(){
        $('[nctype="btn_del_xianshi"]').on('click', function() {
            if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
                var action = "<?php echo urlShopWAP('store_promotion_xianshi', 'xianshi_del');?>";
