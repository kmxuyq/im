...
var Html = {
	...
	fn: function(){
		...
		this.share.innerHTML = shareHtml;	
		new FullScreen({
			class: "videoBox",
...
var News = {
	...
	videoPlayEnd: function(){
		Global.eventUtil.addHandler(News.video, "ended", function(){		
			//$("#video").hide();
			News.video.outerHTML = "";
			News.videoBox.style["opacity"] = 0;
			News.videoBox.style["z-index"] = 1;
			//alert(999);
	...
	timeThrough: function(){
		...
		thruBtn = {
			...
			fn: function(){		
				...
				Global.eventUtil.addHandler(boxEl, "webkitAnimationEnd", function(){
					if(flag==true) return;
					flag = true;					
...
var Main = {
	fn2: function(){
		$("#main2").fadeIn(800, function(){
			new PinchFun({
				id: "main2",
				origin: "160px 185px",
				speed: 800,     //放大速度, 以毫秒为单位
				dwelltime: 2000, //停留时间, 以毫秒为单位
				fn: function(){
					$("#main2").fadeOut(800);
					Main.fn3();
					}
	...
	fn6: function(){
		$("#main6").fadeIn(800, function(){
			Music.audio2.play();
			Global.eventUtil.addHandler(Music.audio2, "ended", function(){
				var Share = function(){
					var El = $("#share");
						El.show();
					if(El.is(":visible")){ 
						El.on("tap", function(){ $(this).hide(); });
						}
					};	
				setTimeout(function(){Share();},1500);
				});
			});
		}
	};//Main{} end
