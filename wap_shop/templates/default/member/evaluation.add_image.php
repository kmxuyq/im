...
<div class="ncm-flow-layout">
  <div class="ncm-flow-container">
    <div class="title"><a href="<?php echo urlShopWAP('member_evaluate', 'list');?>" class="ncm-btn-mini fr"><i class="icon-reply"></i>返&nbsp;回</a>
      <h3>对商品进行晒单</h3>
    </div>
    <div class="alert">
      <h4>图片上传要求：</h4>
      请使用jpg\jpeg\png等格式、单张大小不超过1M的图片，最多可发布5张晒图，上传后的图片也将被保存在个人主页相册中以便其它使用。 </div>
    <form id="add_form" action="<?php echo urlShopWAP('member_evaluate', 'add_image_save');?>" method="post">
      ...
      <div class=" ncm-default-form">
        ...
        <dl>
          ...
          <dd>
          <div>
              <a href="<?php echo urlShopWAP('goods', '', array('goods_id' => $output['geval_info']['geval_goodsid'])); ?>" target="_blank"><img src="<?php echo cthumb($output['geval_info']['geval_goodsimage'], 60);?>"/></a>
              <a href="<?php echo urlShopWAP('goods', '', array('goods_id' => $output['geval_info']['geval_goodsid'])); ?>" target="_blank"><?php echo $output['geval_info']['geval_goodsname'];?></a>
          </div>
          </dd>
        </dl>
        <dl>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(".input-file").fileupload({
            dataType: 'json',
            url: "<?php echo urlShopWAP('sns_album', 'swfupload');?>",
            formData: "",
            add: function(e, data) {
        ...
        function album_pic_del(file_id) {
            var del_url = "<?php echo urlShopWAP('sns_album', 'album_pic_del');?>";
            del_url += '&id=' + file_id;
            $.get(del_url);
        }
