...
$(document).ready(function(){
    ...
    $('#address_form').on('change','select',function(){
        ...
        if (area_id != '-请选择-') {
        	$('#zt_address').load("<?php echo WAP_SHOP_SITE_URL;?>/index.php?act=member_address&op=delivery_list&dlyp_id=<?php echo $output['address_info']['dlyp_id'];?>&area_id="+area_id);
        }
    });

    <?php if (intval($_GET['id'])) { ?>
    $('#zt_address').load("<?php echo WAP_SHOP_SITE_URL;?>/index.php?act=member_address&op=delivery_list&dlyp_id=<?php echo $output['address_info']['dlyp_id'];?>&area_id=<?php echo $output['address_info']['area_id']?>");
