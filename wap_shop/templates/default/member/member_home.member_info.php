  ...
  <div id="security" class="normal">
    <div class="outline">
      <div class="SAM">
        ...
        <a href="<?php echo urlShopWAP('member_security','index');?>" title="安全设置">提升></a>
        <?php } ?>
        </div>
        ...
        <em>已绑定</em>
        <?php  } else {?>
        <a href="<?php echo urlShopWAP('member_security','auth',array('type'=>'modify_mobile'));?>" title="绑定手机">未绑定</a>
        <?php }?></span>
        <span><i class="mail"></i>邮箱：
        <?php if ($output['home_member_info']['member_email_bind'] == 1) {?>
        <em>已绑定</em>
        <?php  } else {?>
        <a href="<?php echo urlShopWAP('member_security','auth',array('type'=>'modify_email'));?>" title="绑定邮箱">未绑定</a>
        <?php }?></span>
        </div>
      </div>
    </div>
