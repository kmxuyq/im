		  ...
		  <?php if($_SESSION['is_login'] == '1'){?>
			<div class="user-joinin">
				  ...
				  <dl>
					...
					<dd> 若您还没有填写入驻申请资料<br/>
					  请点击“<a href="<?php echo urlShopWAP('store_joinin','step0');?>" target="_blank">我要入住</a>”进行入驻资料填写</dd>
					<dd>若您的店铺还未开通<br/>
					  请通过“<a href="<?php echo urlShopWAP('store_joinin','index');?>" target="_blank">查看入驻进度</a>”了解店铺开通的最新状况 </dd>
				  </dl>
				  <div class="bottom"><a href="<?php echo urlShopWAP('store_joinin','step0');?>" target="_blank">我要入住</a><a href="<?php echo urlShopWAP('store_joinin','index');?>" target="_blank">查看入驻进度</a></div>
			</div>
		  <?php }else{?>
			  ...
			  <form id="login_form" action="index.php?act=login" method="post">
				...
				<dl>
				  ...
				  <dd>
					...
					<a href="JavaScript:void(0);" onclick="javascript:document.getElementById('codeimage').src='<?php echo WAP_SHOP_SITE_URL?>/index.php?act=seccode&op=makecode&nchash=<?php echo getNchash();?>&t=' + Math.random();" class="change" title="看不清，换一张">
					<img src="<?php echo WAP_SHOP_SITE_URL?>/index.php?act=seccode&op=makecode&nchash=<?php echo getNchash();?>" class="fl ml5" name="codeimage" id="codeimage" border="0"/></a>
					<label></label>
				  </dd>
				</dl>
				<?php } ?>
				<dl>
				  <dt></dt>
				  <dd>
					<input type="hidden" value="<?php echo WAP_SHOP_SITE_URL;?>/index.php?act=show_joinin" name="ref_url">
					<input name="提交" type="submit" class="button" value="登&nbsp;&nbsp;录">
					<a href="<?php echo urlShopWAP('login','forget_password');?>" target="_blank">忘记密码？</a></dd>
				</dl>
			  </form>
			  <div class="register">还没有成为我们的合作伙伴？ <a href="<?php echo urlShopWAP('login','register');?>" target="_blank">快速注册</a></div>
			</div>
		  <?php }?>
   </div>
