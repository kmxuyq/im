  ...
  <table class="nch-compare-table">
    ...
    <tr id="comparetr_<?php echo $k; ?>" class="goods_tr">
      ...
      <td><dl class="goods-info">
          <dt class="goods-pic"><a href="<?php echo urlShopWAP('goods','index',array('goods_id'=>$v[$i]['goods_id']));?>" target="_blank"><img src="<?php echo cthumb($v[$i]['goods_image'], 240,$v[$i]['store_id']);?>"></a></dt>
          <dd class="goods-name"><a href="<?php echo urlShopWAP('goods','index',array('goods_id'=>$v[$i]['goods_id']));?>" target="_blank"><?php echo $v[$i]['goods_name'];?></a></dd>
          <?php if ($i>0){?>
      ...
      <td><div class="no-compare">
          <h3>暂无对比项</h3>
          <a href="<?php echo urlShopWAP('search','index',array('cate_id'=> $output['cate_id']));?>" title="<?php echo $v['gc_name']; ?>" target="_blank">添加</a></div></td>
      <?php }?>
      <?php }?>
    </tr>
    <?php } ?>
