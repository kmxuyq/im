...
class alipay{
    ...
    public function get_payurl(){
        ...
        $this->parameter = array(
            ...
            'partner'           => $alipay_config['partner'],    //合作伙伴ID
            '_input_charset'    => 'utf-8',    //网站编码
            'sign_type'         => 'MD5',
            'key'               => $alipay_config['key'],
            'notify_url'        => WAP_SHOP_SITE_URL."/api/payment/alipay/notify_url.php",    //通知URL
            'return_url'        => WAP_SHOP_SITE_URL."/api/payment/alipay/return_url.php",    //返回URL
    ...
    public function notify_verify() {
        $param    = $_POST;
        $veryfy_url = $this->alipay_verify_url. "partner=" .$param["partner"]. "&notify_id=".$param["notify_id"];
    ...
    private function create_url() {
        ...
        $url.= $arg."sign=" .urlencode($this->parameter['sign']) ."&sign_type=".$this->parameter['sign_type'];
        return $url;
    }
    ...
    private function sign($parameter) {
        ...
        if($parameter['sign_type'] == 'MD5') {
            ...
            $mysign = md5($prestr);
        }elseif($parameter['sign_type'] =='RSA') {
            //DSA 签名方法待后续开发
            // $private_key = file_get_contents(ALIPAY_PRIVATE_PATH);
