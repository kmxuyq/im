...
class pointcartControl extends BasePointShopControl {
	...
	public function addOp() {
		...
		if(!$_SESSION['is_login']) {
			echo json_encode(array('done'=>false,'msg'=>'您还没登陆,请您登陆','url'=>'index.php?act=member&op=home'));die;
		}
		//验证积分礼品是否存在购物车中
		$model_pointcart = Model('pointcart');
    ...
    public function addressOp() {
        ...
        $address_info = $address_class->getDefaultAddressInfo(array('member_id'=>$_SESSION['member_id']));
        Tpl::output('address_info',$address_info);
        Tpl::showpage('pointcart_address');
    }
	/**
	 * 兑换订单流程第一步
	 */
	public function step1Op(){
		...
		//实例化收货地址模型（不显示自提点地址）
		$address_info = Model('address')->getDefaultAddressInfo(array('member_id'=>$_SESSION['member_id'],'dlyp_id'=>0), 'is_default desc,address_id desc');
		Tpl::output('address_info',$address_info);
