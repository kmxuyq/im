...
class member_orderControl extends BaseMemberControl {
   ...
   public function indexOp() {
      ...
      $condition = array();
      $where     = '1 ';
      $is_share  = intval($_GET['is_share']);
      if (1 == $is_share) {
         $where .= " AND (re_mid_1={$_SESSION['member_id']} OR re_mid_2={$_SESSION['member_id']} OR re_mid_3={$_SESSION['member_id']}) ";
      } else {
         $condition['buyer_id'] = $_SESSION['member_id'];
         $where .= " AND buyer_id={$_SESSION['member_id']} ";
      }
      if ('' != $_GET['order_sn']) {
         $condition['order_sn'] = $_GET['order_sn'];
         $where .= " AND order_sn='{$_GET['order_sn']}' ";
      }
      ...
      if ($start_unixtime || $end_unixtime) {
         ...
         $where .= " AND add_time BETTWEEN {$start_unixtime} AND {$end_unixtime} ";
      }

      if ('' != $_GET['state_type']) {
         $state                    = str_replace(array('state_new', 'state_pay', 'state_send', 'state_success', 'state_noeval', 'state_cancel'), array(ORDER_STATE_NEW, ORDER_STATE_PAY, ORDER_STATE_SEND, ORDER_STATE_SUCCESS, ORDER_STATE_SUCCESS, ORDER_STATE_CANCEL), $_GET['state_type']);
         $condition['order_state'] = $state;
         $where .= " AND order_state={$state} ";
      }
      if ('state_noeval' == $_GET['state_type']) {
         $condition['evaluation_state'] = 0;
         $condition['order_state']      = ORDER_STATE_SUCCESS;
         $where .= " AND evaluation_state=0 AND order_state=" . ORDER_STATE_SUCCESS . " ";
      }

      //回收站
      if ($_GET['recycle']) {
         $condition['delete_state'] = 1;
         $where .= " AND delete_state=1 ";
      } else {
         $condition['delete_state'] = 0;
         $where .= " AND delete_state=0 ";
      }
      $order_list = $model_order->getOrderList($where, 20, '*', 'order_id desc', '', array('order_common', 'order_goods', 'store'));
      ...
      foreach ($order_list as $order_id => $order) {
         ...
         //显示取消订单
         $order['if_cancel'] = $is_share ? 0 : $model_order->getOrderOperateState('buyer_cancel', $order);

         //显示退款取消订单
         $order['if_refund_cancel'] = $is_share ? 0 : $model_order->getOrderOperateState('refund_cancel', $order);

         //显示投诉
         $order['if_complain'] = $is_share ? 0 : $model_order->getOrderOperateState('complain', $order);

         //显示收货
         $order['if_receive'] = $is_share ? 0 : $model_order->getOrderOperateState('receive', $order);

         //显示锁定中
         $order['if_lock'] = $is_share ? 0 : $model_order->getOrderOperateState('lock', $order);

         //显示物流跟踪
         $order['if_deliver'] = $is_share ? 0 : $model_order->getOrderOperateState('deliver', $order);

         //显示评价
         $order['if_evaluation'] = $is_share ? 0 : $model_order->getOrderOperateState('evaluation', $order);

         //显示删除订单(放入回收站)
         $order['if_delete'] = $is_share ? 0 : $model_order->getOrderOperateState('delete', $order);

         //显示永久删除
         $order['if_drop'] = $is_share ? 0 : $model_order->getOrderOperateState('drop', $order);

         //显示还原订单
         $order['if_restore'] = $is_share ? 0 : $model_order->getOrderOperateState('restore', $order);
         ...
         if (!empty($order['extend_order_goods'])) {
            ...
            foreach ($order['extend_order_goods'] as $value) {
               ...
               $value['goods_url']     = urlShopWAP('goods', 'index', array('goods_id' => $value['goods_id']));
               if (5 == $value['goods_type']) {
         ...
         if (ORDER_STATE_NEW == $order['order_state']) {
      ...
      //var_dump($order_group_list);exit;
      //自动收货
      Model('order')->auto_receives_product($_SESSION["member_id"]);
   ...
   public function search_deliverOp() {
      ...
      //--------
      $k_company   = $express[$shipping_express_id]["e_code"];
      $k_number    = $order_info["shipping_code"];
      $kaid100_url = "/wap_shop/index.php?act=member_order&op=get_express&company={$k_company}&number={$k_number}";
      //取得配送公司代码
      //url end

      //print_r($express);
      Tpl::output('kaid100_url', $kaid100_url);
...
//     }
// / }
if (empty($content['data'])) exit(json_encode(false));
if (strtoupper(CHARSET) == 'GBK'){
$output = Language::getUTF8($content['data']);//网站GBK使用编码时,转换为UTF-8,防止json输出汉字问题
}

echo json_encode($content['data']); */
      $k_company = $_GET["company"];
      $k_number  = $_GET["number"];
      $filename  = $_SERVER["DOCUMENT_ROOT"] . '/data/log/order/' . $k_number . '-' . $k_company . '.log';
      if (file_exists($filename)) {
         $content_str = file_get_contents($filename);
         $content     = json_decode($content_str, true);
         // print_r($content);exit();
         // if ($content['status'] != 200) exit(json_encode(false));
         $data = $content['lastResult']['data'];
         ...
         $kaid100 = new kaid100();
         $status  = $kaid100->postorder($k_company, $k_number);
         //log_result ( $status, '-order-status' );
         echo $status;
      }
   }
   ...
   public function show_orderOp() {
      ...
      if (empty($order_info) || ORDER_DEL_STATE_DROP == $order_info['delete_state']) {
         showMessage(Language::get('member_order_none_exist'), '', 'html', 'error');
      }
      ...
      //显示系统自动收获时间
      if (ORDER_STATE_SEND == $order_info['order_state']) {
      ...
      if (ORDER_STATE_CANCEL == $order_info['order_state']) {
      ...
      //------------------
      $k_company = $order_info['express_info']["e_code"];
      $k_number  = $order_info['shipping_code'];
   ...
   public function change_stateOp() {
      ...
      $redirect_url          = '';
      if ('order_cancel' == $_GET['state_type']) {
         $result       = $this->_order_cancel($order_info, $_POST);
         $redirect_url = "index.php?act=member_order&state_type=state_new";
      } else if ('order_receive' == $_GET['state_type']) {
         $result       = $this->_order_receive($order_info, $_POST);
         $redirect_url = "index.php?act=member_order&state_type=state_send";
