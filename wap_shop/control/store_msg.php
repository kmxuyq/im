...
class store_msgControl extends BaseSellerControl {
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array();
        $menu_array = array(
            1 => array('menu_key'=>'msg_list',   'menu_name'=>'消息列表',    'menu_url'=>urlShopWAP('store_msg', 'index')),
            2 => array('menu_key'=>'msg_setting','menu_name'=>'消息接收设置',  'menu_url'=>urlShopWAP('store_msg', 'msg_setting')),
        );
        if (!$this->checkIsAdmin()) {
            unset($menu_array[2]);
        }
