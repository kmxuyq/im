...
class sns_albumControl extends BaseSNSControl {
   ...
   private function profile_menu($menu_type, $menu_key = '') {
      $menu_array     = array();
      $album_menuname = (3 == $this->relation ? Language::get('sns_me') : 'TA') . Language::get('nc_sns_path_my_album');
      switch ($menu_type) {
      case 'album':
         $menu_array = array(
   ...
   public function swfuploadOp() {
      ...
      list($width, $height, $type, $attr) = getimagesizefromstring($post_param['image'][0]);
      ...
      $insert                = array();
      $insert['ap_name']     = $ret['list']['0'];
      $insert['ac_id']       = $class_id;
      $insert['ap_cover']    = C('remote_upload_url');
      ...
      $data                     = array();
      $data['file_id']          = $result;
      $data['file_name']        = $ret['list']['0'];
      $data['origin_file_name'] = $_FILES["file"]["name"];
      $data['file_path']        = C('remote_upload_url');
      $data['file_url']         = C('remote_upload_url') . $ret['list']['0'];
      $data['state']            = 'true';
      /**
       * 整理为json格式
       */
