...
class cartControl extends BaseBuyControl {
	...
	public function indexOp() {
        ...
        $cur_date = date('Ymd', TIMESTAMP);
        $logic_buy_1 = logic('buy_1');
        foreach ($cart_list as $cart) {
            $cart['goods_total'] = ncPriceFormat($cart['goods_price'] * $cart['goods_num']);
            if ($cart['calendar_date']) {
               $tmp = explode(',', $cart['calendar_date']);
               $select_date = str_replace('-', '', $tmp[0]) * 1;
               if ($select_date < $cur_date) {
                  QueueClient::push('delCart', array('buyer_id' => $_SESSION['member_id'], 'cart_ids' => array($cart['cart_id'])));
                  continue;
               }
               $goods = $logic_buy_1->getGoodsOnlineInfo($cart['goods_id'], $cart['goods_num'], $cart['calendar_date']);
		...
		if ($_SESSION['member_id']){
			...
			if(!empty($cart_list)){
				foreach ($cart_list as $k => $cart){
					...
					$cart_array['list'][$k]['goods_url'] = urlShopWAP('goods', 'index', array('goods_id' => $cart['goods_id']));
				}
			}
		} else {
		    //登录前
			...
			foreach ($cart_list as $key => $cart){
				...
				$value['goods_url'] = urlShopWAP('goods', 'index', array('goods_id' => $cart['goods_id']));
	...
	public function addOp() {
        ...
        if (is_numeric($_GET['goods_id'])) {
            ...
            $goods_info['calendar_date'] = trim($_GET['calendar']);
            //抢购
	...
	private function _check_goods($goods_info, $quantity) {
		...
		if ($goods_info['is_fcode'] || $goods_info['is_presell']) {
	...
	public function updateOp() {
		...
		if ($cart_info['bl_id'] == '0') {
		    ...
		    $goods_info	= $logic_buy_1->getGoodsOnlineInfo($goods_id,$quantity, $cart_info['calendar_date']);
		    if(empty($goods_info)) {
    ...
    /**
     * 购物车删除多个商品，未登录前使用cart_id即为goods_id
     */
    public function delcartsOp() {
        ...
        if ($_SESSION['member_id']) {
            //登录状态下删除数据库内容
            $delete = $model_cart->delCartByCartIdString($cart_ids, $_SESSION['member_id']);
            if($delete) {
                $data['state'] = 'true';
                $data['quantity'] = $model_cart->cart_goods_num;
                $data['amount'] = $model_cart->cart_all_price;
