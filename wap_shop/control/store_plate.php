...
class store_plateControl extends BaseSellerControl {
    ...
    public function plate_addOp() {
        if (chksubmit()) {
            ...
            if ($error != '') {
                showDialog(L('error') . $error, urlShopWAP('store_plate', 'index'));
            }
            $insert = array();
            ...
            if ($result) {
                showDialog(L('nc_common_op_succ'), urlShopWAP('store_plate', 'index'),'succ');
            } else {
                showDialog(L('nc_common_op_fail'), urlShopWAP('store_plate', 'index'));
            }
        }
        // 是否能使用编辑器
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'plate_list':
                $menu_array = array(
                    array('menu_key' => 'plate_list', 'menu_name' => '版式列表', 'menu_url' => urlShopWAP('store_plate', 'plate_list'))
                );
                break;
            case 'plate_add':
                $menu_array = array(
                    array('menu_key' => 'plate_list', 'menu_name' => '版式列表', 'menu_url' => urlShopWAP('store_plate', 'plate_list')),
                    array('menu_key' => 'plate_add', 'menu_name' => '添加版式', 'menu_url' => urlShopWAP('store_plate', 'plate_add'))
                );
                break;
            case 'plate_edit':
                $menu_array = array(
                    array('menu_key' => 'plate_list', 'menu_name' => '版式列表', 'menu_url' => urlShopWAP('store_plate', 'plate_list')),
                    array('menu_key' => 'plate_add', 'menu_name' => '添加版式', 'menu_url' => urlShopWAP('store_plate', 'plate_add')),
                    array('menu_key' => 'plate_edit', 'menu_name' => '编辑版式', 'menu_url' => urlShopWAP('store_plate', 'plate_edit'))
                );
                break;
        }
        if(!empty($array)) {
