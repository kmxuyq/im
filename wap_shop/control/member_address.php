...
class member_addressControl extends BaseMemberControl{
	...
	public function addressOp() {
		...
		if (chksubmit()){
			...
			redirect('index.php?act=member_address&op=address');
		}
	...
	public function setDefaultOp(){
		$id = $_POST['id'];
		$model = Model();
		if($model->table('address')->where(array('member_id'=>$_SESSION['member_id'],'is_default'=>1))->update(array('is_default'=>0))){
			if($model->table('address')->where(array('address_id'=>intval($id)))->update(array('is_default'=>1))){
				echo '1';exit();
			}
		}
		echo '0';
