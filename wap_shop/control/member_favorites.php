...
class member_favoritesControl extends BaseMemberControl {
   ...
   public function favoritesgoodsOp() {
      ...
      if (!empty($favorites_info)) {
         //取消收藏
         $favorites_model->where(array('fav_id' => $fav_id, 'fav_type' => 'goods', 'member_id' => $_SESSION['member_id']))->delete();
         $goods_model->editGoodsById(array('goods_collect' => array('exp', 'goods_collect - 1')), $fav_id);
         echo json_encode(array('done' => true, 'msg' => '取消收藏成功','status'=>2));
         die;
      }
      //判断商品是否为当前会员所有
   ...
   public function fglistOp() {
      $favorites_model = Model('favorites');
      $show_type       = 'favorites_goods_index'; //默认为图片横向显示
      $show            = $_GET['show'];
         ...
         if ($result) {
            ...
            if (!empty($fav_arr)) {
               if ('goods' == $type) {
                  ...
                  showDialog(Language::get('favorite_del_success'), 'index.php?act=member_favorites&op=fslist&show=list', 'succ');
               }
            }
         } else {
