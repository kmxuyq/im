...
class weixin_activeControl extends  BaseHomeControl{
	...
	public function indexOp(){
		...
		if(in_array($openid,explode(',', $active_lower_member))){
			...
			if($stock=='0'){
				...
				//如果没有参加过,还有库存
				$start_zj=10;
				$end_zj=intval($GLOBALS["setting_config"]["weixin_zhongjiangli"])+$start_zj;
				$zhongjian_mumber=round(($start_zj+$end_zj)/2);
				$code=rand($start_zj,$end_zj);//rand(10, 19);
				$new_active_lower_member=$active_lower_member.$openid.',';

				//$code=='15'
				if($code==$zhongjian_mumber){
					//中奖
	...
	public function wx_present_selOp(){
		...
		$active_rule_list=Model()->query("SELECT b.goods_image,b.store_id,a.* from {$tablepre}wx_active_rule as a LEFT JOIN {$tablepre}goods as b on a.goods_id=b.goods_id order by lower_num asc");
		$openid=decrypt($_GET["openid"]);
	 ...
	 * 中奖礼券领取地址
	 */
	public function wx_voucher_selOp(){
		if(!empty($_GET["openid"])){
			$openid=decrypt($_GET["openid"]);
			$this->login($openid, 'openid');
			//$present_member_id=$_GET["present_member_id"];
			//$lower_num=decrypt($_GET["lower"]);
			$voucher_url='/wap_shop/index.php?act=member_voucher';
			if(isset($_SESSION["member_id"])){
				callmsg('礼券已下发到您的卡包', $voucher_url);
			}else{
				$this->login($openid, 'openid');
				callmsg('礼券已下发到您的卡包，请进入个人中心我的礼券查看', $voucher_url);
			}
		}
	}
	/**
	...
	public function wx_present_success2Op(){
		...
		Tpl::output('message',$message);
		Tpl::showpage('wx_present_success2','null_layout');
	}
	...
	public function testOp(){
		$openid='ocuxlxHi73NfaP4ls4N_3MC_iJJg';
		$this->login($openid, 'openid');
		exit();
