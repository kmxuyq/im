...
class member_snsindexControl extends BaseMemberControl {
	...
	public function indexOp(){
		...
		//信息输出
		Tpl::output('nchash',substr(md5(WAP_SHOP_SITE_URL.$_GET['act'].$_GET['op']),0,8));
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($result){
				...
				$content_str .= "<div class=\"fd-media\">
					<div class=\"goodsimg\"><a target=\"_blank\" href=\"".urlShopWAP('goods', 'index', array('goods_id'=>$goods_info['goods_id']))."\"><img src=\"".thumb($goods_info, 240)."\" onload=\"javascript:DrawImage(this,120,120);\" alt=\"{$goods_info['goods_name']}\"></a></div>
					<div class=\"goodsinfo\">
						<dl>
							<dt><a target=\"_blank\" href=\"".urlShopWAP('goods', 'index', array('goods_id'=>$goods_info['goods_id']))."\">".$goods_info['goods_name']."</a></dt>
				...
				if (C('share_isuse') == 1){
					...
					$params['url'] = urlShopWAP('goods' , 'index', array('goods_id'=>$goods_info['goods_id']));
	...
	public function sharestoreOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($result){
				...
				$store_info['store_url'] = urlShopWAP('show_store', 'index', array('store_id'=>$store_info['store_id']));
				$content_str .= "<div class=\"fd-media\">
				...
				if (C('share_isuse') == 1){
				    ...
				    $params['url'] = urlShopWAP('show_store' , 'index', array('store_id'=>$store_info['store_id']));
	...
	public function tracelistOp(){
		...
		if (!empty($tracelist)){
			foreach ($tracelist as $k=>$v){
				if ($v['trace_title']){
					$v['trace_title'] = str_replace("%siteurl%", WAP_SHOP_SITE_URL.DS, $v['trace_title']);
				...
				if(!empty($v['trace_content'])){
					//替换内容中的siteurl
					$v['trace_content'] = str_replace("%siteurl%", WAP_SHOP_SITE_URL.DS, $v['trace_content']);
				}
				$tracelist[$k] = $v;
			}
		}
