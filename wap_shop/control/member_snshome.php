...
class member_snshomeControl extends BaseSNSControl {
	...
	public function indexOp(){
		...
		if (!empty($storelist)){
			...
			if (!empty($goodslist)){
				foreach ($goodslist as $k=>$v){
					$v['goodsurl'] = urlShopWAP('goods', 'index', array('goods_id'=>$v['goods_id']));
	...
	public function goodsinfoOp(){
		...
		$sharegoods_info['snsgoods_goodsurl'] = urlShopWAP('goods', 'index', array('goods_id'=>$sharegoods_info['snsgoods_goodsid']));
	...
	public function traceinfoOp(){
		...
		if (!empty($tracelist)){
			...
			if ($traceinfo['trace_title']){
				$traceinfo['trace_title'] = str_replace("%siteurl%", WAP_SHOP_SITE_URL.DS, $traceinfo['trace_title']);
			...
			if(!empty($traceinfo['trace_content'])){
				//替换内容中的siteurl
				$traceinfo['trace_content'] = str_replace("%siteurl%", WAP_SHOP_SITE_URL.DS, $traceinfo['trace_content']);
			}
		}
