...
class Control
{
    ...
    protected function checkWxLogin()
    {
        //微信登陆
        //1、没有没有登陆的用户需要走登陆流程
        //2、带了store_member_info参数需要走登陆流程
        if ('1' !== $_SESSION['is_login'] || intval($_GET['store_member_info'])) {
            $inWeixin = $this->checkIsInWeixin();
            if ($inWeixin) {
                // if($_GET['code']){
                //    var_dump($_SESSION);exit;
                // }
                if ($_GET['code']) {
                    //$ref_url  = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING'];
                    $store_id = intval($_GET['state']);
                    $res      = $this->wxloginbycode($_GET['code'], $store_id);
                    //微信免登陆成功，重新定义 ref_url
                    $url = urldecode($_SESSION['ref_url']);
                    //去除的&store_member_info = ''之后的字符串
                    $url_new             = explode("&store_member_info=$store_id", $url);
                    $_SESSION['ref_url'] = urlencode($url_new[0]);
                    //同时将店铺ID赋值到seesion里面
                    $_SESSION['store_member_info_ID'] = $store_id;
                    redirect($url_new[0]);
                } else {
                    ...
                    $ref_url             = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING'];
                    $_SESSION['ref_url'] = urlencode($ref_url);
                    //对应附带store_id
                    $store_id = intval($_GET['store_member_info']);
                    if (empty($store_id)) {
                        die('使用微信抓取信息,请附带店铺参数');
                    }

                    $state             = $store_id;
                    $_SESSION['state'] = $state;
                    $url               = $this->wxloginurl($ref_url, $state);
                    redirect($url);
                }
            }
        }
    ...
    private function wxloginurl($ref_url, $state = '')
    {
        //如果说的是免登陆，一定要带参数store_member_info的参数，否则无法获取对应的appid和appseret;
        $info = Model()->table('store_wxinfo')->field('appid')->where(array('store_id' => $state))->find();
        if (empty($info)) {
            die("请配置对应的店铺$state的微信信息");
        }

        $appid = $info['appid'];
        //$appid      = $GLOBALS["setting_config"]["weixin_appid"];
        $uri        = urlencode($ref_url);
        $wxloginurl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . $appid . '&redirect_uri=' . $uri . '&response_type=code&scope=snsapi_userinfo&state=' . $state . '#wechat_redirect';
        return $wxloginurl;
    }
    private function wxloginbycode($code, $store_member_info = '')
    {
        //根据 $store_member_info找到appid,appsecret
        $info = Model()->table('store_wxinfo')->field('appid,appsecret')->where(array('store_id' => $store_member_info))->find();
        if (empty($info)) {
            die("请配置对应的店铺$store_member_info的微信信息");
        }

        $appid     = $info['appid'];
        $appsecret = $info['appsecret'];
//       $appid     = $GLOBALS["setting_config"]["weixin_appid"];
        //       $appsecret = $GLOBALS["setting_config"]["weixin_appsecret"];
        ...
        if (!empty($result['openid'])) {
            ...
            $store_member_model = Model()->table('store_member');
            $member_where       = array('openid' => $result['openid']);
            //调整
            $store_member = $store_member_model->where($member_where)->find();
            if (!empty($store_member)) {
                $member = $member_model->getMemberInfo(array('member_id' => $store_member['member_id']));
                if (!empty($member)) {
                    return $member_model->createSession($member);
                } else {
                    die('没有找到store_member中' . $store_member['member_id'] . '对应的会员信息');
                }
            } else {
                //查到cookie
                if (!empty($_COOKIE['mctoken'])) {
                    $deb       = new desceb();
                    $mc_member = $deb->new_decrypt($_COOKIE['mctoken']);
                    $member    = $member_model->getMemberInfo(array('member_id' => $mc_member));
                    if (!empty($member)) {
                        //塞入store_member
                        $store_member_model->insert(array(
                            'store_id' => $store_member_info, 'openid' => $result['openid'], 'member_id' => $mc_member,
                        ));
                        return $member_model->createSession($member);
                    }
                }
                //新建az_member
                $uri       = sprintf('https://api.weixin.qq.com/sns/userinfo?access_token=%s&openid=%s&lang=zh_CN', $result['access_token'], $result['openid']);
                $ret       = curl($uri);
                $user_data = json_decode($ret, true);
                // echo '<pre>';var_dump($user_data);exit;
                if ($user_data['errcode'] > 0) {
                    die('get user info failed');
                } else {
                    $register_info['username'] = preg_replace_callback('/./u', function (array $m) {
                        return strlen($m[0]) >= 4 ? '' : $m[0];
                    }, strval($user_data['nickname']));
                    $register_info['password']        = '';
                    $register_info['email']           = '';
                    $register_info['inviter_id']      = 0;
                    $register_info['member_avatar']   = strval($user_data['headimgurl']);
                    $register_info['member_sex']      = intval($user_data['sex']);
                    $register_info['member_areainfo'] = '';
                    $register_info['openid']          = strval($user_data['openid']);
                    $register_info['member_mobile']   = '';
                    $member_info                      = $member_model->weixin_register($register_info);
                    if (empty($member_info)) {
                        die('create user failed');
                    }
                    //塞入store_member
                    $store_member_model->insert(array(
                        'store_id'  => $store_member_info,
                        'openid'    => $result['openid'],
                        'member_id' => $member_info['member_id'],
                    ));
                    return $member_model->createSession($member_info);
                }
            }
        } else {
            //回到上一页
            header('Location: ' . $_SERVER['HTTP_REFERER']);
            //die('get userinfo error');
        }
        return array();
    }

    private function createAvatar($url, $openid)
    {
        if (empty($url)) {
            return '';
        }
        ...
        $file      = $openid . '_weixin.jpg';
        $hash      = md5($file);
        $rand      = substr($hash, 5, 2);
        $face_url  = $face_path . $rand . '/' . $file;
        //如果头像不存在，则自动生成
        if (!file_exists($face_url)) {
            $tmp_file_name = $openid . '_tmp.jpg';
            $local_img     = $face_path . $tmp_file_name;
            if (!file_exists($face_path . $rand)) {
                @mkdir($face_path . $rand, 0777, true);
            }

            $ch = curl_init();
            $fp = fopen($local_img, 'wb');
...
class BaseHomeControl extends Control
{
    ...
    public function __construct()
    {
        ...
        if (intval($_GET['store_id'])) {
            $_SESSION['route_store_id'] = intval($_GET['store_id']);
        }
        if ($_SESSION['route_store_id']) {
            Tpl::output('route_store_id', $_SESSION['route_store_id']);
        } else {
            Tpl::output('route_store_id', $_SESSION['store_id']);
        }
        //用户当前处于的页面状态（分销||正常微商城）
...
class BaseMemberControl extends Control
{
    ...
    public function __construct()
    {
        ...
        //     $_SESSION['member_id'] = intval($_GET['user_id']);
        // } else {
        //     exit('会员ID信息错误！');
        // }
        $_SESSION['member_id'] = 77180;
        if ($_GET['column'] && strtoupper(CHARSET) == 'GBK') {
            $_GET = Language::getGBK($_GET);
        }
        $this->checkWxLogin();
        $iswx = $this->checkIsInWeixin();
        if (!$iswx) {
            //会员验证
            // $this->checkLogin();
        }
        ...
        //获得会员信息
        $this->member_info = $this->getMemberAndGradeInfo(true);
        // $this->member_info['member_avatar'] = Model()->table('wx_member')->field('headimgurl')->where('member_id = '.$this->member_info['member_id'])->find();
    ...
    protected function _getCommonOperationsAndNavLink($act = '')
    {
        ...
        // 面包屑
        $nav_link   = array();
        $nav_link[] = array('title' => L('homepage'), 'link' => WAP_SHOP_SITE_URL);
        if ('' == $nav) {
            $nav_link[] = array('title' => '我的商城');
        } else {
            $nav_link[] = array('title' => '我的商城', 'link' => urlShopWAP('member', 'home'));
    ...
    private function _getMenuList()
    {
        $menu_list = array(
            'trade' => array('name' => '交易管理', 'child' => array(
                'member_order'     => array('name' => '实物交易订单', 'url' => urlShopWAP('member_order', 'index')),
                'member_vr_order'  => array('name' => '虚拟兑码订单', 'url' => urlShopWAP('member_vr_order', 'index')),
                'member_favorites' => array('name' => '我的收藏', 'url' => urlShopWAP('member_favorites', 'fglist')),
                'member_evaluate'  => array('name' => '交易评价/晒单', 'url' => urlShopWAP('member_evaluate', 'list')),
                'predeposit'       => array('name' => '账户余额', 'url' => urlShopWAP('predeposit', 'pd_log_list')),
                'member_flea'      => array('name' => '我的闲置', 'url' => urlShopWAP('member_flea', 'index')),
                'member_points'    => array('name' => '我的积分', 'url' => urlShopWAP('member_points', 'index')),
                'member_voucher'   => array('name' => '我的代金券', 'url' => urlShopWAP('member_voucher', 'index')),
            )),
            'serv'  => array('name' => '客户服务', 'child' => array(
                'member_refund'      => array('name' => '退款及退货', 'url' => urlShopWAP('member_refund', 'index')),
                'member_complain'    => array('name' => '交易投诉', 'url' => urlShopWAP('member_complain', 'index')),
                'member_consult'     => array('name' => '商品咨询', 'url' => urlShopWAP('member_consult', 'my_consult')),
                'member_inform'      => array('name' => '违规举报', 'url' => urlShopWAP('member_inform', 'index')),
                'member_mallconsult' => array('name' => '平台客服', 'url' => urlShopWAP('member_mallconsult', 'index')),
            )),
            'info'  => array('name' => '资料管理', 'child' => array(
                'member_information' => array('name' => '账户信息', 'url' => urlShopWAP('member_information', 'member')),
                'member_security'    => array('name' => '账户安全', 'url' => urlShopWAP('member_security', 'index')),
                'member_address'     => array('name' => '收货地址', 'url' => urlShopWAP('member_address', 'address')),
                'member_message'     => array('name' => '我的消息', 'url' => urlShopWAP('member_message', 'message')),
                'member_snsfriend'   => array('name' => '我的好友', 'url' => urlShopWAP('member_snsfriend', 'find')),
                'member_goodsbrowse' => array('name' => '我的足迹', 'url' => urlShopWAP('member_goodsbrowse', 'list')),
                'member_connect'     => array('name' => '第三方账号登录', 'url' => urlShopWAP('member_connect', 'qqbind')),
                'member_sharemanage' => array('name' => '分享绑定', 'url' => urlShopWAP('member_sharemanage', 'index')),
            )),
            'app'   => array('name' => '应用管理', 'child' => array(
                'sns'       => array('name' => '个人主页', 'url' => urlShopWAP('member_snshome', 'index')),
...
class BaseSNSControl extends Control
{
    ...
    private function check_relation()
    {
        ...
        if ('1' == $_SESSION['is_login']) {
            if ($this->master_id == $_SESSION['member_id']) {
                ...
                if (!empty($friend_arr) && 2 == $friend_arr['friend_followstate']) {
                    $this->relation = 2;
                } elseif (1 == $friend_arr['friend_followstate']) {
                    $this->relation = 4;
                }
            }
        }
    ...
    private function add_visit()
    {
        $model = Model();
        //记录访客
        if ('1' == $_SESSION['is_login'] && 3 != $this->relation) {
            //访客为会员且不是空间主人则添加访客记录
...
class BaseStoreControl extends Control
{
    ...
    public function __construct()
    {
        ...
        if ($store_info['store_decoration_switch'] > 0 & 1 == $store_info['store_decoration_only']) {
...
class BaseSellerControl extends Control
{
    ...
    public function __construct()
    {
        ...
        if ('seller_login' !== $_GET['act']) {
            ...
            if (1 !== $_SESSION['seller_is_admin'] && 'seller_center' !== $_GET['act'] && 'seller_logout' !== $_GET['act']) {
...
class BasePointShopControl extends Control
{
    ...
    public function __construct()
    {
        ...
        if (C('points_isuse') != 1 || C('pointshop_isuse') != 1) {
            showMessage(Language::get('pointshop_unavailable'), urlShopWAP('index', 'index'), 'html', 'error');
        }
...
class sharebaseControl extends control
{
    protected $member_info = array(); // 会员信息
    ...
    public function __construct()
    {
        ...
        //输出头部的公用信息
        $this->showLayout();
        Tpl::setDir('shareshop');
        Tpl::setLayout('shareshop_layout');
        ...
        if (empty($store_id)) {
            redirect('/wap_shop/index.php?act=index');
        }
        $store_info = Model('store')->getStoreInfoByID($store_id);
        if (empty($store_info)) {
            redirect('/wap_shop/index.php?act=index');
        }
        $_SESSION['store_info'] = $store_info;
        #微信会员信息
        if ($_SESSION['member_id']) {
            $this->wx_member = Model('wx_member')->where(array('member_id' => $_SESSION['member_id'], 'store_id' => $store_id))->find();
        }

        $_SESSION['store_id']   = $store_id;
        $_SESSION['share_shop'] = 1;

        #基础设置信息
        $this->settings = Model('share_settings')->where(array('store_id' => $store_id))->find();
        #分销关系锁定
        $pid = intval($_GET['share_uid']);
        if ($pid > 0) {
            $this->lockRelationship($pid);
        }
        if (intval($_GET['store_id'])) {
            $_SESSION['route_store_id'] = intval($_GET['store_id']);
        }
    ...
    public function lockRelationship($pid)
    {
        ...
        $member_info = $share_model->where(array('store_id' => $_SESSION['store_id'], 'member_id' => $_SESSION['member_id']))->find();
        #上级是否存在
        if ($pid > 0) {
        ...
        switch ($this->settings['expire']) {
            case '1':
                ...
                #永久锁定关系
                $share_model->insert(array(
                    'member_id'  => $this->member_info['member_id'],
                    'openid'     => $this->member_info['openid'],
                    'reg_time'   => TIMESTAMP,
                    'nickname'   => $this->member_info['member_name'],
                    'sex'        => $this->member_info['member_sex'],
                    'headimgurl' => $this->member_info['member_avatar'],
                    'pid'        => $pid,
                    'store_id'   => $_SESSION['store_id'],
