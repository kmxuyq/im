...
class store_navigationControl extends BaseSellerControl {
    ...
    public function navigation_editOp() {
        ...
        if($sn_id <= 0) {
           showMessage(L('wrong_argument'), urlShopWAP('store_navigation', 'navigation_list'), '', 'error');
        }
        $model_store_navigation = Model('store_navigation');
    ...
    public function navigation_saveOp() {
        ...
        showDialog(L('nc_common_op_succ'), urlShopWAP('store_navigation', 'navigation_list'), 'succ');
    }
    ...
    public function navigation_delOp() {
        ...
        if($sn_id > 0) {
            ...
            showDialog(L('nc_common_op_fail'), urlShopWAP('store_navigation', 'navigation_list'), 'error');
        }
    }
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array[] = array(
            ...
            'menu_name' => '导航列表',
            'menu_url' => urlShopWAP('store_navigation', 'navigation_list')
        );
        if($menu_key == 'navigation_add') {
            $menu_array[] = array(
                'menu_key' => 'navigation_add',
                'menu_name' => '添加导航',
                'menu_url' => urlShopWAP('store_navigation', 'navigation_add')
            );
        }
        if($menu_key == 'navigation_edit') {
            $menu_array[] = array(
                ...
                'menu_name' => '编辑导航',
                'menu_url' => urlShopWAP('store_navigation', 'navigation_edit')
            );
        }
