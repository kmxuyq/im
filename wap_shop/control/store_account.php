...
class store_accountControl extends BaseSellerControl {
    ...
    public function account_addOp() {
        ...
        if (empty($seller_group_list)) {
            showMessage('请先建立账号组', urlShopWAP('store_account_group', 'group_add'), '', 'error');
        }
    ...
    public function account_saveOp() {
        ...
        if($result) {
            ...
            showDialog(Language::get('nc_common_op_succ'), urlShopWAP('store_account', 'account_list'), 'succ');
        } else {
            $this->recordSellerLog('添加账号失败');
            showDialog(Language::get('nc_common_save_fail'), urlShopWAP('store_account', 'account_list'), 'error');
        }
    }
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array[] = array(
            ...
            'menu_name' => '账号列表',
            'menu_url' => urlShopWAP('store_account', 'account_list')
        );
        if($menu_key === 'account_add') {
            $menu_array[] = array(
                'menu_key'=>'account_add',
                'menu_name' => '添加账号',
                'menu_url' => urlShopWAP('store_account', 'account_add')
            );
        }
        if($menu_key === 'account_edit') {
            $menu_array[] = array(
                ...
                'menu_name' => '编辑账号',
                'menu_url' => urlShopWAP('store_account', 'account_edit')
            );
        }
