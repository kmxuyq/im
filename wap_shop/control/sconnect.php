...
class sconnectControl extends BaseHomeControl{
	...
	public function registerOp(){
		...
		if($result) {
			...
			$ref_url = cookie('ref_url');
			if(!empty($ref_url)) {
				$ref_url = urldecode($ref_url);
			} else {
				$ref_url = WAP_SHOP_SITE_URL;
			}
			setNcCookie('ref_url','',-3600);
			$model_member->createSession($member_info);
			$success_message = Language::get('login_usersave_regist_success');
			showMessage($success_message, $ref_url);
		} else {
			showMessage(Language::get('login_usersave_regist_fail'),WAP_SHOP_SITE_URL.'/index.php?act=login&op=register','html','error');
		}
	}
	/**
	 * 新浪微博账号绑定已有用户
	...
	public function changesinaOp(){
		...
		header('Location:'.WAP_SHOP_SITE_URL.'/api.php?act=tosina');
		exit;
	}
}
