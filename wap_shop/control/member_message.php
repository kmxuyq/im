...
class member_messageControl extends BaseMemberControl {
	...
	public function sendmsgOp(){
		...
		}else if($_GET['namestr'] != ''){
			Tpl::output('member_name',$_GET['namestr']);
		}
		/* //批量给好友发放站内信页面
	 ...
	 *显示好友列表
	 */
	 public function showfriendOp(){
		 //批量给好友发放站内信页面
		$friend_model = Model('sns_friend');
		$friend_list = $friend_model->listFriend(array('friend_frommid'=>"{$_SESSION['member_id']}"));
		if($_GET['namestr'] != ''){
			$namearr = explode(',',$_GET['namestr']);
			foreach($friend_list as $k => $v){
				
				if(in_array($v['friend_tomname'],$namearr)){
				 $friend_list[$k] = array_merge($friend_list[$k],array('checked'=>'checked'));					
				}else{
					$friend_list[$k] = array_merge($friend_list[$k],array('checked'=>''));	
				}
		...
		Tpl::output('friend_list',$friend_list);
		Tpl::showpage('member_message.friend');
	 }
	/**
	 * 站内信保存操作
	 *
	...
	public function showmsgcommonOp() {
		...
		Model()->table('message')->where(array('message_id'=>$message_id))->update(array('message_state'=>3));//更新为已读状态，3为已读
		//更新未读站内信数量cookie值
	...
	private function profile_menu($menu_key='') {
		$menu_array	= array(
		    ...
		    5=>array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>urlShopWAP('member_message', 'setting'))
		);
		if($menu_key == 'sendmsg') {
