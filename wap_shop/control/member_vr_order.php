...
class member_vr_orderControl extends BaseMemberControl {
   ...
   public function indexOp() {
      ...
      if (1 == $is_share) {
         $where .= " AND (re_mid_1={$_SESSION['member_id']} OR re_mid_2={$_SESSION['member_id']} OR re_mid_3={$_SESSION['member_id']}) and pay_sn=0";
      } else {
      ...
      $order_list = $model_vr_order->getOrderList($where, 20, '*', 'order_id desc');
      ...
      foreach ($order_list as $key => $order) {
         ...
         //显示取消订单
         $order_list[$key]['if_cancel'] = $is_share ? 0 : $model_vr_order->getOrderOperateState('buyer_cancel', $order);

         //显示支付
         $order_list[$key]['if_pay'] = $is_share ? 0 : $model_vr_order->getOrderOperateState('payment', $order);

         //显示删除订单(放入回收站)
         $order_list[$key]['if_delete'] = $is_share ? 0 : $model_vr_order->getOrderOperateState('delete', $order);

         //显示永久删除
         $order_list[$key]['if_drop'] = $is_share ? 0 : $model_vr_order->getOrderOperateState('drop', $order);

         //显示还原订单
         $order_list[$key]['if_restore'] = $is_share ? 0 : $model_vr_order->getOrderOperateState('restore', $order);

         //显示退款
         $order_list[$key]['if_refund'] = $is_share ? 0 : $model_vr_order->getOrderOperateState('refund', $order);

         //显示评价
         $order_list[$key]['if_evaluation'] = $is_share ? 0 : $model_vr_order->getOrderOperateState('evaluation', $order);

         //显示分享
         $order_list[$key]['if_share'] = $is_share ? 0 : $model_vr_order->getOrderOperateState('share', $order);
   ...
   public function change_stateOp() {
      ...
      if (!$result['state']) {
         ...
         redirect('index.php?act=member_vr_order&state_type=state_new');
      }
   }
   ...
   public function resendOp() {
      ...
      $condition['buyer_id'] = $_SESSION['member_id'];
      $order_info            = $model_vr_order->getOrderInfo($condition);
      if (empty($order_info) && ORDER_STATE_PAY != $order_info['order_state']) {
         showDialog('订单信息发生错误');
      }
