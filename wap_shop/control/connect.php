...
class connectControl extends BaseHomeControl{
	...
	public function registerOp(){
		...
		if (chksubmit()){
			...
			showMessage(Language::get('nc_common_save_succ'),WAP_SHOP_SITE_URL);
		}else {
			//检查登录状态
			...
			if($result) {
				...
				showMessage(Language::get('login_usersave_regist_fail'),WAP_SHOP_SITE_URL.'/index.php?act=login&op=register','html','error');//"会员注册失败"
			}
		}
	}
	/**
	...
	public function autologin(){
		...
		if (is_array($member_info) && count($member_info)>0){
			...
			showMessage($success_message,WAP_SHOP_SITE_URL);
		}
	}
	/**
	 * 更换绑定QQ号码
	...
	public function changeqqOp(){
		...
		unset($_SESSION['openid']);
		@header('Location:'.WAP_SHOP_SITE_URL.'/api.php?act=toqq');
		exit;
	}
}
