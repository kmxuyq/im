...
class store_promotion_bundlingControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('promotion_allow')) !== 1) {
            showMessage(Language::get('promotion_unavailable'), urlShopWAP('seller_center', 'index'),'','error');
        }
    ...
    public function bundling_quota_addOp() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog(Language::get('bundling_quota_price_fail'), urlShopWAP('store_promotion_bundling', 'bundling_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_bundling = Model('p_bundling');
            ...
            if ($return) {
                ...
                showDialog(L('bundling_quota_price_succ'), urlShopWAP('store_promotion_bundling', 'bundling_list'), 'succ');
            } else {
                showDialog(L('bundling_quota_price_fail'), urlShopWAP('store_promotion_bundling', 'bundling_quota_add'));
            }
        }
        // 输出导航
    ...
    public function bundling_addOp() {
        ...
        if (chksubmit()) {
            ...
            showDialog(L('nc_common_op_succ'), urlShopWAP('store_promotion_bundling', 'bundling_list'), 'succ');
        }
        ...
        if (intval($_GET['bundling_id']) > 0) {
            ...
            if (empty($bundling_info['store_id'])) {
                showMessage(L( 'wrong_argument'), urlShopWAP('store_promotion_bundling', 'bundling_list'), '', 'error' );
            }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'bundling_quota_list':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_list'))
                );
                break;
            case 'bundling_quota_add':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_quota_add', 'menu_name'=>Language::get('bundling_quota_add'), 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_quota_add'))
                );
                break;
            case 'bundling_renew':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_renew'))
                );
                break;
            case 'bundling_add':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_add', 'menu_name'=>Language::get('bundling_add'), 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_add'))
                );
                break;
            case 'bundling_edit':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Language::get('bundling_list'), 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_edit', 'menu_name'=>Language::get('bundling_edit'), 'menu_url'=>urlShopWAP('store_promotion_bundling', 'bundling_edit'))
                );
            break;
        }
