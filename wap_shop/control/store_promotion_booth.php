...
class store_promotion_boothControl extends BaseSellerControl {
    ...
    public function booth_quota_addOp() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlShopWAP('store_promotion_booth', 'booth_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_booth = Model('p_booth');
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlShopWAP('store_promotion_booth', 'booth_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlShopWAP('store_promotion_booth', 'booth_quota_add'));
            }
        }
        // 输出导航
    ...
    public function booth_renewOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlShopWAP('store_promotion_booth', 'booth_list'), 'succ');
            } else {
                showDialog('购买失败', urlShopWAP('store_promotion_booth', 'booth_quota_add'));
            }
        }
    ...
    public function choosed_goodsOp() {
        ...
        $goods_info['url'] = urlShopWAP('goods', 'index', array('goods_id' => $goods_info['goods_id']));
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'booth_goods_list':
                $menu_array	= array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShopWAP('store_promotion_booth', 'booth_goods_list'))
                );
                break;
            case 'booth_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShopWAP('store_promotion_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlShopWAP('store_promotion_booth', 'booth_quota_add'))
                );
                break;
            case 'booth_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlShopWAP('store_promotion_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlShopWAP('store_promotion_booth', 'booth_renew'))
                );
                break;
        }
