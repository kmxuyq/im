...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function groupbuy_quota_add_saveOp() {
        ...
        showDialog(Language::get('groupbuy_quota_add_success'), urlShopWAP('store_groupbuy', 'groupbuy_list'), 'succ');
    }
    ...
    public function groupbuy_addOp() {
        ...
        if (checkPlatformStore()) {
            ...
            $current_groupbuy_quota = $model_groupbuy_quota->getGroupbuyQuotaCurrent($_SESSION['store_id']);
            if(empty($current_groupbuy_quota)) {
                showMessage('当前没有可用套餐，请先购买套餐',urlShopWAP('store_groupbuy', 'groupbuy_quota_add'),'','error');
            }
            Tpl::output('current_groupbuy_quota', $current_groupbuy_quota);
        }
    ...
    public function groupbuy_saveOp() {
        ...
        if (!checkPlatformStore()) {
            ...
            $current_groupbuy_quota = $model_groupbuy_quota->getGroupbuyQuotaCurrent($_SESSION['store_id']);
            if(empty($current_groupbuy_quota)) {
                showDialog('当前没有可用套餐，请先购买套餐',urlShopWAP('store_groupbuy', 'groupbuy_quota_add'),'error');
            }
        }
    ...
    public function groupbuy_goods_infoOp() {
        ...
        $data['goods_href'] = urlShopWAP('goods', 'index', array('goods_id' => $goods_info['goods_id']));
    ...
    private function profile_menu($menu_key='') {
        $menu_array	= array(
            1=>array('menu_key'=>'groupbuy_list','menu_name'=>L('nc_member_path_group_list'),'menu_url'=>urlShopWAP('store_groupbuy', 'groupbuy_list'))
        );
        switch ($menu_key){
        case 'groupbuy_add':
        ...
        case 'groupbuy_quota_add':
            $menu_array[] = array('menu_key'=>'groupbuy_quota_add','menu_name'=>'购买套餐','menu_url'=>urlShopWAP('store_groupbuy', 'groupbuy_quota_add'));
            break;
        case 'groupbuy_edit':
