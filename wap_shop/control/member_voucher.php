...
class member_voucherControl extends BaseMemberControl{
	public function __construct() {
		...
		if (intval(C('voucher_allow')) !== 1){
			showMessage(Language::get('member_voucher_unavailable'),urlShopWAP('member', 'home'),'html','error');
		}
	}
	/*
	 * 默认显示代金券模版列表
    ...
    public function voucher_listOp(){
    	...
    	Model('wx_member')->member_voucher($_SESSION['member_id']);

		$model = Model('voucher');
        $list = $model->getMemberVoucherListPro($_SESSION['member_id'], $_GET['select_detail_state'], 10);
      //   if($_GET['select_detail_state']==2) {
      //       $_list = $model->getMemberVoucherListPro($_SESSION['member_id'], 3, 10);
