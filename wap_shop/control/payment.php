...
class paymentControl extends BaseHomeControl{
	...
	public function returnOp(){
		...
		$out_trade_no = $_GET['out_trade_no'];//out_trade_no=$order_info["order_sn"] 商品订单号
		$trade_no = $_GET['trade_no'];
		$url = WAP_SHOP_SITE_URL.'/index.php?act='.$act;
		...
		//支付成功后跳转
		if ($order_type == 'real_order') {
		    $pay_ok_url = WAP_SHOP_SITE_URL.'/index.php?act=buy&op=pay_ok&pay_sn='.$out_trade_no.'&pay_amount='.encrypt($api_pay_amount);//ncPriceFormat
		} elseif ($order_type == 'vr_order') {
		    $pay_ok_url = WAP_SHOP_SITE_URL.'/index.php?act=buy_virtual&op=pay_ok&order_sn='.$out_trade_no.'&order_id='.$order_pay_info['order_id'].'&order_amount='.encrypt($api_pay_amount);//ncPriceFormat
		} elseif ($order_type == 'pd_order') {
		    $pay_ok_url = WAP_SHOP_SITE_URL.'/index.php?act=predeposit';
		}
