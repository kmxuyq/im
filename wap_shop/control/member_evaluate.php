    ...
    public function addOp(){
        ...
        if (!$_POST){
            ...
            foreach ($order_goods as $value){
                ...
                //eval_image
                if(!empty($_POST['goods'][$value['goods_id']]['eval_image'])) {
                    $evaluate_image = implode(',', $_POST['goods'][$value['goods_id']]['eval_image']);
                } else {
                    $evaluate_image = '';
                }
...
//             //添加店铺评价
            ...
            showDialog(Language::get('member_evaluation_evaluat_success'),'index.php?act=member&op=home', 'succ');
        }
    }
...
//             }
    ...
    public function add_image_saveOp() {
        ...
        $goods_url = urlShopWAP('goods', 'index', array('goods_id' => $geval_info['geval_goodsid']));
        //同步到sns
        $content = "
            <div class='fd-media'>
        ...
        if($result) {
            showDialog(L('nc_common_save_succ'), urlShopWAP('member_evaluate', 'list'), 'succ');
        } else {
            showDialog(L('nc_common_save_succ'), urlShopWAP('member_evaluate', 'list'));
        }
    }
}
