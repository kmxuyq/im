...
class member_informationControl extends BaseMemberControl
{
    ...
    {
        $this->memberOp();
//        Tpl::showpage('member_information.index');
    }
    /**
     * 我的资料【用户中心】
     *
    ...
    public function memberOp()
    {
        ...
        if (chksubmit()) {
            ...
            if (strlen($_POST['birthday']) >= 8 and strlen($_POST['birthday']) <= 10) {
    ...
    public function uploadOp()
    {
        ...
        if (!empty($_FILES['pic']['tmp_name'])) {
            ...
            if (!$result) {
                ...
                $src        = BASE_UPLOAD_PATH . DS . ATTACH_AVATAR . DS . "avatar_{$_SESSION['member_id']}_new.$ext";
                $avatarfile = BASE_UPLOAD_PATH . DS . ATTACH_AVATAR . DS . "avatar_{$_SESSION['member_id']}.jpg";
                if (is_file($avatarfile)) {
                    @unlink($avatarfile);
                }
                rename($src, $avatarfile);
