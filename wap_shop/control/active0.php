...
//         echo $m.$g;
    ...
    public function thepost() {
		...
		$condition =array();
                    ...
                    //微信活动
                    if($is_true){//生成订单
                        $member = Model()->table('member')->field('member_id,member_name,member_email')->where(array('member_id'=>$member_id))->find();
                        if (empty($member['member_email'])){
                            $member['member_email'] = 'ymtx@126.com';
                        }
                        $logic_buy = Logic('buy');
                        $result = $logic_buy->buyStep1(array('0'=>$condition['goods_id'].'|1'), false, $member_id, 0);

                        $az_ok = $logic_buy->changeAddr($result['freight_list'], $data['city_id'], $condition['address_id'], $member_id);

                        $post = array();

                        $post['cart_id'][] = $condition['goods_id'].'|1';
                        $post['pay_message'][7] = '';
                        $post['ifcart'] = '';
                        ...
                        $post['invoice_id'] = '';
                        $post['ref_url'] = '';
                        $result = $logic_buy->buyStep2($post, $member_id, $member['member_name'], $member['member_email']);
                        echo json_encode($result);
                        Model()->table('wx_present_member')->where(array('id'=>$_POST['present_member_id']))->update(array('goods_id'=>$az['goods_id'],'status'=>'1'));
                    }                    
    ...
    public function testOp() {
        ...
        $result = $logic_buy->buyStep1(array('0'=>'104509|1'), false, $member['member_id'], 0);
...
/*
        //cartidd
        $post['cart_id']  = array('0'=>$condition['goods_id'].'|1');
        $post['pay_message'] = array('7'=>'');
        $post['password'] = "";
        $post['password_callback'] = "";
        $post['ifcart'] = 1;
        $post['pay_name'] = 'online';
        $post['vat_hash'] = $result['data']['vat_hash'];
        $post['address_id'] = $condition['address_id'];
        ...
        $post['offpay_hash'] = $az_ok['offpay_hash'];
        $post['offpay_hash_batch'] =  $az_ok['offpay_hash_batch'];
        $post['invoice_id'] = $result['data']['inv_info']['inv_id'];
        $post['ref_url'] = '';*/
	 $result1 = $logic_buy->buyStep2($post, $member['member_id'], $member['member_name'], $member['member_email']);
var_dump($result1);
    }
}
