...
class show_groupbuyControl extends BaseHomeControl {
   ...
   public function __construct() {
      ...
      if (intval(C('groupbuy_allow')) !== 1) {
         showMessage(Language::get('groupbuy_unavailable'), urlShopWAP(), '', 'error');
      }
   ...
   public function select_cityOp() {
      ...
      if (0 != $city_id && (!isset($this->groupbuy_vr_cities['name'][$city_id])
         || !isset($this->groupbuy_vr_cities['parent'][$city_id])
         || 0 != $this->groupbuy_vr_cities['parent'][$city_id])) {
         showMessage('该城市不存在，请选择其他城市');
      }

      setNcCookie('city_id', $city_id);

      redirect(urlShopWAP('show_groupbuy', $_GET['back_op']));
   }
   ...
   private function _show_vr_groupbuy_list($function_name) {
      ...
      $groupbuy_order     = '2' == $_GET['groupbuy_order'] ? 'desc' : 'asc';
   ...
   //即将开始的限时折扣
   public function xianshi_zk_soonOp() {
      Tpl::output('current', 'soon');
      Tpl::output('buy_button', '未开始');
      $this->_show_xianshi_zk_buy_list('getGroupbuySoonList');
   }
   //进行中的限时折扣
   public function xianshi_zk_listOp() {
      Tpl::output('current', 'online');
      Tpl::output('buy_button', L('groupbuy_buy'));
      $this->_show_xianshi_zk_buy_list('getGroupbuyOnlineList');
   }
   //已经结束的限时折扣
   public function xianshi_zk_historyOp() {
      Tpl::output('current', 'history');
      Tpl::output('buy_button', '已结束');
      $this->_show_xianshi_zk_buy_list('getGroupbuyHistoryList');
   }
   //获取限时折扣列表
   private function _show_xianshi_zk_buy_list($function_name) {
      $model_xianshi = Model('p_xianshi_goods');
      $store_id      = !empty($_SESSION['store_id']) ? $_SESSION['store_id'] : $_SESSION['route_store_id'];
      $condition     = array(
         'store_id' => $store_id,
      );
      $order              = '';
      $goods_xianshi_info = Model('p_xianshi_goods')->$function_name($condition, 20, $order);
      Tpl::output('groupbuy_list', $goods_xianshi_info);
      Tpl::output('show_page', $model_xianshi->showpage(5));
      ...
      Tpl::showpage('xianshi_zk_list');
   }
   //获取限时折扣的详情信息
   public function xianshi_detailOp() {
      $group_id = intval($_GET['group_id']);
      if (empty($group_id)) {
         showMessage(Language::get('param_error'), urlShopWAP('show_groupbuy', 'index'), '', 'error');
      }
      $model_groupbuy = Model('p_xianshi_goods');
      //   $condition = array('xianshi_goods_id'=>$group_id);
      //   $xianshi_info = $model_groupbuy ->getXianshiGoodsExtendList($condition);
      $xianshi_info = $model_groupbuy->getXianshiGoodsInfoByID($group_id);
      if (empty($xianshi_info)) {
         showMessage('抢购不存在', urlShopWAP('show_groupbuy', 'index'), '', 'error');
      }
      $xianshi_info = $model_groupbuy->getXianshiGoodsExtendInfo($xianshi_info);
      Tpl::output('groupbuy_info', $xianshi_info);
      $goods_detail = Model('goods')->getWapGoodsDetail($xianshi_info['goods_id']);
      if (empty($goods_detail)) {
         showMessage('商品不存在或已下架', urlShopWAP('show_groupbuy', 'index'), '', 'error');
      }
      $goods_spec  = $goods_detail['goods_info']['goods_spec'];
      if(!empty($goods_spec)){
         $goods_spec = array_values($goods_spec);
         $package    = $goods_spec[0];
         Tpl::output('package', $package);
      }
      // echo '<pre>';var_dump($xianshi_info);exit;
      Tpl::output('goods', $goods_detail['goods_info']);
      Tpl::output('goods_id', $xianshi_info['goods_id']);
