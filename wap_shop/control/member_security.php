...
class member_securityControl extends BaseMemberControl {
   ...
   public function send_bind_emailOp() {
      ...
      $verify_url = WAP_SHOP_SITE_URL . '/index.php?act=login&op=bind_email&uid=' . $uid . '&hash=' . md5($seed);
   ...
   public function authOp() {
      ...
      if (!empty($_POST)) {
         //校验验证码
         if (!in_array($_POST['type'], array('modify_pwd', 'modify_mobile', 'modify_email', 'modify_paypwd', 'pd_cash', 'cmms'))) {
         ...
         if ($member_common_info['auth_code'] != $_POST['auth_code'] || TIMESTAMP - $member_common_info['send_acode_time'] > 18000) {
         ...
         if ('pd_cash' == $_POST['type']) {
            $share_settings = Model('share_settings')->find();
            Tpl::output('share_settings', $share_settings);
            Tpl::showpage('member_pd_cash.add');
         } elseif('cmms' == $_POST['type']){
            redirect('/wap_shop/index.php?act=member&op=cmmsapply');
         } else {
            //print_r($_POST);
            //print_r($member_common_info );exit();
            Tpl::showpage('member_security.' . $_POST['type']);
         }

      } else {
         if (!in_array($_GET['type'], array('modify_pwd', 'modify_mobile', 'modify_email', 'modify_paypwd', 'pd_cash', 'cmms'))) {
            redirect('index.php?act=member_security&op=index');
         }
         ...
         if (('modify_email' == $_GET['type'] && '0' == $member_info['member_email_bind']) ||
            ('modify_mobile' == $_GET['type'] && '0' == $member_info['member_mobile_bind'])) {
         ...
         if (in_array($_GET['type'], array('modify_pwd', 'modify_paypwd')) && '0' == $member_info['member_email_bind'] && '0' == $member_info['member_mobile_bind']) {
   ...
   public function send_auth_codeOp() {
      ...
      } elseif ('mobile' == $_GET['type']) {
         //$sms = new Sms();
         //$result = $sms->send($member_info["member_mobile"],$message);
         $sms    = new SendCms();
         $result = $sms->send_code($member_info["member_mobile"], $verify_code);
      }
      if ($result) {
   ...
   public function modify_pwdOp() {
      ...
      if (TIMESTAMP - $_SESSION['auth_modify_pwd'] > 18000) {
   ...
   public function modify_paypwdOp() {
      ...
      //身份验证后，需要在30分钟内完成修改密码操作
      if (TIMESTAMP - $_SESSION['auth_modify_paypwd'] > 18000) {
   ...
   public function modify_mobileOp() {
      ...
      if (chksubmit()) {
         ...
         $obj_validate->validateparam = array(
            ...
            array("input" => $_POST["auth_code"], "require" => "true", 'validator' => 'number', "message" => '请正确填写手机验证码'),
         );
         $error = $obj_validate->validate();
         if ('' != $error) {
            showValidateError($error);
         ...
         $condition['auth_code'] = intval($_POST['auth_code']);
         ...
         if (TIMESTAMP - $member_common_info['send_acode_time'] > 18000) {
   ...
   public function send_modify_mobileOp() {
      //echo json_encode(array('state'=>'true','msg'=>$_GET["mobile"]));exit();
      $obj_validate                = new Validate();
      $obj_validate->validateparam = array(
      ...
      //---
      $sms    = new SendCms();
      $result = $sms->send_code($_GET["mobile"], $verify_code);
      if ($result) {
   ...
   public function testOp() {
      Tpl::showpage('member_security.modify_mobile');
   }
}
