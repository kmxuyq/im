...
class member_complainControl extends BaseMemberControl{
    ...
    public function complain_saveOp() {
      ...
      //   echo '<pre>';var_dump($pic_name);exit;
        $input['complain_pic1'] = $pic_name[0];
        $input['complain_pic2'] = $pic_name[1];
        $input['complain_pic3'] = $pic_name[2];
    ...
    public function get_complain_talkOp() {
        ...
        foreach($complain_talk_list as $talk) {
            $talk_list[$i]['time'] = date("Y-m-d H:i:s",$talk['talk_datetime']);
			$talk_list[$i]['name'] = $talk['talk_member_name'];
            ...
            if(intval($talk['talk_state']) === 2) {
                $talk_list[$i]['content'] = Language::get('talk_forbit_message');
            }else{
				$talk_list[$i]['content'] = $talk['talk_content'];
			}
			$i++;
        }
    ...
    private function upload_pic() {
      $refund_pic    = array();
      $refund_pic[1] = 'input_complain_pic1';
      $refund_pic[2] = 'input_complain_pic2';
      $refund_pic[3] = 'input_complain_pic3';
