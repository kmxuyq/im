...
class searchControl extends BaseHomeControl {
    ...
    public function product_listOp(){
    	...
    	$orderby_str='goods_click';
    	$desc_str='desc';
    	$condition='goods_state=1';//下架的商品不显示
		$store_id = (isset($_GET['store_id']) and !empty($_GET['store_id'])) ? intval($_GET['store_id']) : ($_SESSION['store_id']?:$_SESSION['route_store_id']);
		$condition.= " and store_id = $store_id";
    	//关键字
    	if(!empty($_GET["keyword"])){
    		$keyword=trim($_GET["keyword"]);
    		if(is_numeric($keyword)&&strlen($keyword)>5){
    			$condition.=" and goods_id={$keyword}";
    		}else{
    	...
    	if(!empty($_GET['key'])&&!empty($_GET['order'])){
    		if($_GET["key"]=='1'){
    			$orderby_str='goods_salenum';
    		}elseif($_GET["key"]=='2'){
    			$orderby_str='goods_click';
    		}elseif($_GET["key"]=='3'){
    			$orderby_str='goods_price';
    		}
    		//------------
    		if($_GET["order"]=='1'){
    			$desc_str='asc';
    	...
    	if(!empty($_GET["cate_id"])){
    		...
    		$condition.=" and (gc_id={$cate_id} or gc_id_1={$cate_id} or gc_id_2={$cate_id} or gc_id_3={$cate_id})";
    	}
        if($_GET['type']=='active'
        || $_GET['type']=='appaly'){
            $n_condition = '';
        }else {
            $n_condition = ' and gc_id_1!=1';
        }

    	/*if($_GET['type']=='appaly'){    //申领判断
    	$con = "active.type=0 and active.member_id=".$_SESSION['member_id'];
    	$field ="active.goods_id,active.member_id,active.az_active_state";
    	$active_list =Model()->table('goods,active')->join('left join')->on('active.goods_id=goods.goods_id')->field($field)->where($con)->select();
        foreach ($active_list as $key => $value) {
            $active_list[$value['goods_id']] = $value;
        }
    	Tpl::output('active_list', $active_list);
        }*/
		if($_GET['type']=='appaly'){    //申领判断
    	$con = "active.type=0 and active.goods_id=goods.goods_id and active.member_id=".$_SESSION['member_id'];
    	$field ="active.goods_id,active.member_id,active.az_active_state";
    	...
    	$total_res=$model->query("select count('goods_id') from az_goods where ".$condition.$n_condition." group by goods_commonid");
    	$total=count($total_res);
//    	$goods_list=$model->where($condition.$n_condition)->group('goods_commonid')->page(10,$total)->order("{$orderby_str} {$desc_str}")->select();
    	$goods_list=$model->where($condition.$n_condition)->group('goods_commonid')->order("{$orderby_str} {$desc_str}")->select();

    	//商品分类
    	$product_type=Model()->table('goods_class')->where(array('gc_parent_id'=>$_GET['cate_id']))->select();
//        var_dump($condition.$n_condition);exit();
    	Tpl::output('goods_list',$goods_list);
    	Tpl::output('product_type',$product_type);
		Tpl::output('store_id',$store_id);
    	if($total>10){
    		$page = $model->showpage(6);
    	...
    	if($_GET['type'] == "magic"){
    	    ...
    	    Tpl::showpage('act_product_list');
    	}if($_GET['type'] == "appaly") {
            Tpl::showpage('appaly_list');
        }else{
//			Tpl::showpage('search_1');
            switch (trim($_GET['cate_id'])) {
                case '1126':
                    Tpl::output('title',"包车");
                    break;
                case '1127':
            ...
            $order_url = "/wap_shop/index.php?act=search&op=product_list&cate_id=".$_GET['cate_id']."&store_id=".$store_id;
            Tpl::output('order_url',$order_url);
            Tpl::showpage('ticket_list','null_layout');
        }
	...
	public function share_product_listOp(){

    	$orderby_str='goods_click';
    	$desc_str='desc';
    	$condition='goods_state=1';//下架的商品不显示
		$condition .= " and isshare = 1";
	...
	*商品列表
	*/
	public function product_list0Op() {
...
//print_r($goods_class_array);exit;
        //优先从全文索引库里查找
