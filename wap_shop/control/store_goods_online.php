...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_save_goodsOp() {
        ...
        if (!chksubmit() || $common_id <= 0) {
            showDialog(L('store_goods_index_goods_edit_fail'), urlShopWAP('store_goods_online', 'index'));
        }
        // 验证表单
        ...
        if ($error != '') {
            showDialog(L('error') . $error, urlShopWAP('store_goods_online', 'index'));
        }
    ...
    public function edit_imageOp() {
        ...
        if ($common_id <= 0) {
            showMessage(L('wrong_argument'), urlShopWAP('seller_center'), 'html', 'error');
        }
        $model_goods = Model('goods');
    ...
    public function edit_save_imageOp() {
        if (chksubmit()) {
            ...
            if ($common_id <= 0 || empty($_POST['img'])) {
                showDialog(L('wrong_argument'), urlShopWAP('store_goods_online', 'index'));
            }
            $model_goods = Model('goods');
            // 删除原有图片信息
            ...
            $this->recordSellerLog('编辑商品，平台货号：'.$common_id);
                ...
                showDialog(L('nc_common_save_fail'), urlShopWAP('store_goods_online', 'index'));
            }
        }
    }
    ...
    public function get_goods_list_ajaxOp() {
        ...
        foreach ($goods_list as $key => $val) {
            ...
            $goods_list[$key]['url'] = urlShopWAP('goods', 'index', array('goods_id' => $val['goods_id']));
        }
    ...
    private function profile_menu($menu_type,$menu_key, $allow_promotion = array()) {
        ...
        switch ($menu_type) {
            case 'goods_list':
                $menu_array = array(
                   array('menu_key' => 'goods_list',    'menu_name' => '出售中的商品', 'menu_url' => urlShopWAP('store_goods_online', 'index'))
                );
                break;
            case 'edit_detail':
                if ($allow_promotion['lock'] === false) {
                    $menu_array = array(
                        array('menu_key' => 'edit_detail',  'menu_name' => '编辑商品', 'menu_url' => urlShopWAP('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                        array('menu_key' => 'edit_image',   'menu_name' => '编辑图片', 'menu_url' => urlShopWAP('store_goods_online', 'edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())))),
                    );
                }
                if ($allow_promotion['gift']) {
                    $menu_array[] = array('menu_key' => 'add_gift', 'menu_name' => '赠送赠品', 'menu_url' => urlShopWAP('store_goods_online', 'add_gift', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer()))));
                }
                if ($allow_promotion['combo']) {
                    $menu_array[] = array('menu_key' => 'add_combo', 'menu_name' => '推荐组合', 'menu_url' => urlShopWAP('store_goods_online', 'add_combo', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer()))));
                }
                break;
            case 'edit_class':
                $menu_array = array(
                    array('menu_key' => 'edit_class',   'menu_name' => '选择分类', 'menu_url' => urlShopWAP('store_goods_online', 'edit_class', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_detail',  'menu_name' => '编辑商品', 'menu_url' => urlShopWAP('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_image',   'menu_name' => '编辑图片', 'menu_url' => urlShopWAP('store_goods_online', 'edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())))),
                );
                break;
