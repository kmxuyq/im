...
class activityModel{
	...
	private function getCondition($condition){
		...
		if($condition['activity_id'] != ''){
			if(is_array($condition['activity_id'])){
				$conditionStr	.= " and activity.activity_id {$condition['activity_id'][0]} ";
			}else{
		...
		if($condition['activity_type'] != ''){
			//modify
			if(is_array($condition['activity_type'])){
				$conditionStr.= " and activity.activity_type in (";
				foreach ($condition['activity_type'] as $value ){
					$conditionStr.="'".$value."',";
				}
