...
$tpl_spec['urlwap'] = urlShopWap('goods', 'index', array('goods_id' => $value['goods_id']));
class goodsModel extends Model{
	...
	//
	public function getGoodsCommonListForVrPromotion2($condition, $field = '*', $page = 10) {
        $condition['is_virtual']    = 1;
        $condition['is_fcode']      = 0;
        $condition['is_presell']    = 0;
        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
		//
		return $this->getGoodsList($condition, $field, '','', '', $page, '', '');
     ...
     * @return array
     */
    public function getGoodCommonsList($condition, $field = '*') {
     ...
     * @return array
     */
    public function getGoodInfoNotOnSaleById($goods_id) {
     ...
     * @return array
     */
    public function getGoodsNotOnSaleListByIdArray($goodsid_array) {
        if (empty($goodsid_array) || !is_array($goodsid_array)) return array();

        $goods_list = array();
        foreach ($goodsid_array as $goods_id) {
            $goods_info = $this->getGoodInfoNotOnSaleById($goods_id);
            if (!empty($goods_info)) $goods_list[$goods_info['goods_id']] = $goods_info;
        }
     ...
     * @return array
     */
    public function getWapGoodsDetail($goods_id) {
        if($goods_id <= 0) {
            return null;
        }
        $result1 = $this->getGoodsInfoAndPromotionById($goods_id);
        ...
        foreach ($spec_array as $key => $value) {
            ...
            $tpl_spec = array();
            $tpl_spec['sign'] = $spec_sign;
            $tpl_spec['url'] = urlShopWAP('goods', 'index', array('goods_id' => $value['goods_id']));
            $spec_list[] = $tpl_spec;
            $spec_list_mobile[$spec_sign] = $value['goods_id'];
            $spec_image[$value['color_id']] = thumb($value, 60);
        }
        ...
        $goods_info['cart'] = true;
        //虚拟、F码、预售不显示加入购物车
        if ($goods_info['is_fcode'] == 1 || $goods_info['is_presell'] == 1) {
            $goods_info['cart'] = false;
        }
	...
	public function getMobileBodyByCommonID($goods_commonid, $fields = 'mobile_body') {
     ...
     /**
      * 获取高尔夫 stock_info，包括套餐价格
      * @param  $goods_info,字段：goods_price,spec_value
      * @param  $commonid
      * @param  $date 2015-10-05
      * @param  $golf_site_arr=unserialize($GLOBALS["setting_config"]["golf_site"]);
	  * @param  $golf_minute_arr=unserialize($GLOBALS["setting_config"]["golf_minute"]);
      * @param$stock默认为1
      */
     public function get_golf_stock_info($goods_info,$commonid,$date,$golf_site_arr,$golf_minute_arr,$stock='1'){
     	$man_price=$goods_info["goods_price"];
     	//-------------
     	//库存套餐价格HOME

     	if(is_array($goods_info["spec_value"])){
     		$spec_value_arr=$goods_info["spec_value"];
     	}else{
     		$spec_value_arr=unserialize($goods_info["spec_value"]);
     	}
     	$spec_value=array_values($spec_value_arr);
     	//print_r($spec_value);

     	foreach($spec_value[0] as $v){
     		$price_count=Model()->table('golf_price')->where(array('commonid'=>$commonid,'package'=>$v))->count();
     		if($price_count=='0'){
     			$price_arr=array($man_price,$man_price);
     		}else{
     			$price=Model()->table('golf_price')->where(array('commonid'=>$commonid,'package'=>$v))->field('price')->find();
     			$price_arr= explode('|', $price["price"]);
     		}
     		//判断是否是周末，周末则调用假日价格
     		$week=date('w',strtotime($date));
     		//echo '----'.$week;
     		if($week=='6'||$week=='0'){
     			$week_price=$price_arr[1];
     		}else{
     			$week_price=$price_arr[0];
     		}
     		$package_price[$v]=$week_price;//--------------------------
     		//echo $v.'--';
     	}
     	//库存套餐价格END
     	...
     	foreach($golf_minute_arr as $v2){
     		//获取价格
     		$golf_minute[$v2]=array('price'=>$package_price,'stock'=>$stock);
     	}
     	foreach($golf_site_arr as $v){
     		$stock_info[$v]=$golf_minute;
     	}
     	//stock_info  END
     	return $stock_info;
     }
     /**
      * 高尔夫，没有库存，自动插入本月库存
      * @param  $goods_info商品表goods_common信息，字段spec_value,goods_price：,可以为空
      * @param  $commonid商品commonid
      * @param  $where_stock查询库存表（golf）where条件数组（commonid,date_month）
      * @param  $date当前日期，格式：2015-10
      * @param  $stock库存，默认1
      */
     public function insert_golf_stock_info($goods_info,$commonid,$where_stock,$date,$stock='1'){
     	$date_month=substr($date,0,7);
     	$where_stock["date_month"]=$date_month;
     	$count=Model()->table('golf')->where($where_stock)->count();

     	//没有库存，自动插入本月库存
     	if($count=='0'){
	     	if(empty($goods_info)){
	     		$goods_info=Model()->table('goods_common')->where(array('goods_commonid'=>$commonid))->field('spec_value,goods_price')->find();
	     	}
     		$days_curent=date("t", mktime(0, 0, 0, substr($date,5,2), 1, substr($date,0,4)));//本月天数
     		//stock_info  HOME
     		$golf_site_arr=unserialize($GLOBALS["setting_config"]["golf_site"]);
     		$golf_minute_arr=unserialize($GLOBALS["setting_config"]["golf_minute"]);
     		$stock_num=count($golf_site_arr)*count($golf_minute_arr);
     		//stock_info  END
     		for($i=1;$i<=$days_curent;$i++){
     			$golf_insert_data[]=array('commonid'=>$commonid,'date_month'=>$date_month,'day'=>$i);
     			$golf_stock_date=date('Y-m-d',strtotime($date_month.'-'.$i));
     			//获取stock_info
     			$stock_info=$this->get_golf_stock_info($goods_info, $commonid, $golf_stock_date, $golf_site_arr, $golf_minute_arr);
     			$golf_stock_insert_data[]=array('commonid'=>$commonid,'date'=>$golf_stock_date,'stock'=>$stock_num,'stock_info'=>serialize($stock_info));
     		}
     		//print_r($golf_insert_data);
     		//print_r($golf_stock_insert_data);
     		//批量插入数据golf表
     		Model()->table('golf')->insertAll($golf_insert_data);
     		//批量插入数据golf_stock表
     		Model()->table('golf_stock')->insertAll($golf_stock_insert_data);
     		//插入结束
     	}
     	return '1';
     }
     public function fake_sale_num($goods_sale_num) {
        $goods_sale_num = intval($goods_sale_num);
        if($goods_sale_num<=200) {
            return $goods_sale_num*3;
        } elseif($goods_sale_num<=300) {
            return 600+($goods_sale_num-200)*4;
        } else {
            return 1000+($goods_sale_num-300);
        }
     }
}
