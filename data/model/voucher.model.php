...
class voucherModel extends Model {
    ...
    public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0) {
       ...
       foreach ($voucher_list as $key => $voucher) {
           if ($goods_total < $voucher['voucher_limit']) {
               ...
               $voucher_list[$key]['desc'] = sprintf('满%s元,减%s元 有效期至 %s',intval($voucher['voucher_limit']),$voucher['voucher_price'],date('Y-m-d',$voucher['voucher_end_date']));
           }
       }
       return $voucher_list;
    }
	 ...
	 * @return string
	 */
	public function getCurrentAvailableNoLimitVoucher($condition = array(), $goods_total = 0) {
	   $condition['voucher_end_date'] = array('gt',TIMESTAMP);
	   $condition['voucher_state'] = 1;
	   $voucher_list = $this->table('voucher')->where($condition)->key('voucher_t_id')->order('voucher_limit asc')->select();
	   foreach ($voucher_list as $key => $voucher) {
	           $voucher_list[$key]['desc'] = sprintf('满%s减%s有效期至%s',intval($voucher['voucher_limit']),$voucher['voucher_price'],date('Y-m-d',$voucher['voucher_end_date']));
	   }
	   return $voucher_list;
	}
     ...
     * @param int $page 分页数
     */
    public function getMemberVoucherListPro($member_id, $voucher_state, $page = null) {
        if(empty($member_id)) {
            return false;
        }
        ...
        $field = 'voucher_id,voucher_code,voucher_title,voucher_desc,voucher_start_date,voucher_end_date,voucher_price,voucher_limit,voucher_state,voucher_order_id,voucher_store_id,voucher_t_customimg';
        ...
        $list = $this->table('voucher,voucher_template')->field($field)->join('left')->on('voucher.voucher_t_id = voucher_template.voucher_t_id')->where($where)->order('voucher_id desc')->page($page)->select();
