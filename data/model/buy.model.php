...
//             QueueClient::push('delCart', array('buyer_id'=>$member_id,'cart_ids'=>array_keys($input_buy_items)));
    ...
    /**
     * 外部使用代金券 buy.logic.php调用 
     * @param unknown_type $voucher_code
     */
	public function up_input_voucher_list($voucher_code){
		//将代金券状态设为已使用，重新计算商品总价并赋值		
		$voucher_res=Model()->table('voucher')->where(array('voucher_code'=>$voucher_code,'voucher_state'=>1))->field('voucher_store_id,voucher_id,voucher_t_id,voucher_price')->find();
		if(!empty($voucher_res["voucher_id"])){
			$store_id=$voucher_res["voucher_store_id"];
			$input_voucher_list[$store_id]['voucher_t_id']=$voucher_res["voucher_t_id"];
			$input_voucher_list[$store_id]['voucher_price']=$voucher_res["voucher_price"];
			$input_voucher_list[$store_id]['voucher_id']=$voucher_res["voucher_id"];
			$input_voucher_list[$store_id]['voucher_code']=$voucher_code;
			$input_voucher_list[$store_id]['voucher_owner_id']=$_SESSION["member_id"];//将代金券设为用户所有
			Model()->table("voucher")->where(array('voucher_id'=>$voucher_res["voucher_id"]))->update(array('voucher_owner_id'=>$_SESSION["member_id"]));
	 ...
	 * 使用代金券后更新表order_id
	 */
	public function up_voucher_order_id($order_id,$voucher_id){
		$status=Model()->table("voucher")->where(array('voucher_id'=>$voucher_id))->update(array('voucher_order_id'=>$order_id));
		return $status;
	}
}
