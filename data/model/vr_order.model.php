...
class vr_orderModel extends Model {
 ...
 /**
     ...
     * @return unknown
     */
    public function getAllOrderInfo($condition = array(), $fields = '*', $master = false) {
        $order_info = $this->table('vr_order')->field($fields)->where($condition)->master($master)->select();
        if (empty($order_info)) {
            return array();
        }
        if (isset($order_info['order_state'])) {
     ...
     * @return  int();返回  insert_id
     */
    public  function addGolfOrder($data){
      return $insert = $this->table('golf_order')->insert($data);
    }
...
/**
     ...
     * @param string $fields
     */
    public function getAllOrderCodeInfo($condition = array(), $fields = '*',$master = false) {
        return $this->table('vr_order_code')->field($fields)->where($condition)->master($master)->select();
    }
     ...
     * @return array
     */
    public function getRecordList($condition, $pagesize = '', $field = '*', $order = 'vr_usetime desc', $limit = ''){
        $list = $this->table('vr_order')->field($field)->where($condition)->page($pagesize)->order($order)->limit($limit)->select();
        if (empty($list)) return array();
        foreach ($list as $key => $order) {
            if (isset($order['order_state'])) {
