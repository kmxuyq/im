...
class orderModel extends Model {
    ...
    public function getBuy($condition){
        return $this->table('order_goods')->where($condition)->find();
    }
     ...
     * @return unknown
     */
    public function auto_receives_product($member_id=''){
    	$buyer_id_str='';
    	if(!empty($member_id)){
    		$buyer_id_str=" and order.buyer_id={$member_id}";
    	}
    	$end_time=time()-86400+7;//7天
    	$data=array("order_common.order_state"=>40);
    	$list=Model()->table('order,order_common')->where("order.order_id=order_common.order_id and order_common.shipping_time<{$end_time} and order.order_state=30 {$buyer_id_str}")->field('order.order_id')->select();
    	if(count($list)>0){
	    	foreach($list as $v){
	    		$state=Model()->table('order')->where(array('order_id'=>$v["order_id"]))->update(array('order_state'=>40));
	    	}
    	}
    	return $state;
    }
     ...
     * @return array
     */
    public function getShiWuTotalByStatorder($where, $field = '*', $order = '', $group = '') {
        return $this->table('order')->field($field)->where($where)->group($group)->order($order)->find();
    }
    /**
     * 查询虚拟订单统计
     *
     ...
     * @return array
     */
    public function getVrTotalByStatorder($where, $field = '*', $order = '', $group = '') {
        return $this->table('vr_order')->field($field)->where($where)->group($group)->order($order)->find();
    }
