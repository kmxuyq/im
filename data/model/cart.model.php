...
class cartModel extends Model {
	...
	private function _addCartDb($goods_info = array(),$quantity) {
    	...
    	if (!empty($check_cart)) {
         if(isset($goods_info['calendar_date']) and !empty($goods_info['calendar_date'])) {
            $array    = array();
            ...
            $array['calendar_date'] = $goods_info['calendar_date'];
            $this->where(['cart_id' => $check_cart['cart_id'], 'buyer_id' => $goods_info['buyer_id']])->update($array);
         }
         return true;
      }
	...
	private function _addCartCookie($goods_info = array(), $quantity = null) {
    	...
    	if (in_array($goods_info['goods_id'],array_keys($cart_array)) and !(isset($goods_info['calendar_date']) and !empty($goods_info['calendar_date']))) {
         return true;
      }
		$cart_array[$goods_info['goods_id']] = array(
		  ...
		  'goods_num' => $quantity,
        'calendar_date' => $goods_info['calendar_date'],
		);
     ...
     * @param int $buyer_id
     * @param unknown_type $conditio
     */
    public function delCartByCartIdString($cart_ids, $buyer_id) {
        $cartids = explode(',', $cart_ids);
        if (empty($cartids)) {
            return false;
        }
        $result = $this->delCart('db',array('cart_id' => array('in', $cartids), 'buyer_id'=>$buyer_id));
        return $result;
    }
