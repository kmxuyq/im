...
class goods_classModel extends Model
{
     ...
     * @return array $nav_link 返回数组形式类别导航连接
     */
    public function getGoodsClassGfNav($id = 0, $sign = 1) {
        if (intval ( $id ) > 0) {
            $data = $this->getGoodsClassIndexedListAll();
            ...
            if ($sign == 1) {
                ...
                $nav_link [] = array(
                        'title' => $data[$id]['gc_name'],
                        'link' => '/?act=search&op=index&cate_id='.$data[$id]['gc_id'],
                        //'link' => urlShop('search', 'index', array('cate_id' => $data[$id]['gc_id']))
                );
            }
        ...
        // 首页导航
        $nav_link[] = array('title' => Language::get('homepage'), 'link' => 'http://www.gellefreres.com/');
