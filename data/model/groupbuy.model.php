...
class groupbuyModel extends Model{
    ...
    public function addGroupbuy($param){
        ...
        $param['state'] = self::GROUPBUY_STATE_NORMAL;
        $param['recommended'] = 0;
    ...
    public function getGroupbuyExtendInfo($groupbuy_info) {
		...
		$gs = Model()->table('goods')->where(array('goods_id'=>$groupbuy_info['goods_id']))->find();
		$groupbuy_info['goods_imageurl'] = UPLOAD_SITE_URL.'/shop/store/goods/'.$gs['store_id'].'/'.$gs['goods_image'];
		//
		$gsimgs = Model('goods_images')->where(array('goods_commonid'=>$gs['goods_commonid']))->select();
		foreach($gsimgs as $ks=>$vals){
			$groupbuy_info['goods_images_url'][$ks] = UPLOAD_SITE_URL.'/shop/store/goods/'.$gs['store_id'].'/'.$vals['goods_image'];
		}
		$groupbuy_info['groupbuy_name'] = $groupbuy_info['groupbuy_name'];
		$groupbuy_info['gids'] = $groupbuy_info['goods_id'];
		$groupbuy_info['goods_name'] = $groupbuy_info['goods_name'];
		$groupbuy_info['zongxu'] = intval($groupbuy_info['total_quantity']);
		$groupbuy_info['yigou'] = intval($groupbuy_info['buy_quantity']);
		$groupbuy_info['shengyu'] = intval($groupbuy_info['total_quantity'])-intval($groupbuy_info['buy_quantity']);
		...
		$groupbuy_info['end_time_text2'] = date('Y-m-d H:i', ($groupbuy_info['start_time']+($groupbuy_info['group_shixian']*3600)));
		$groupbuy_info['end_time_shixian'] = $groupbuy_info['group_shixian'];
		$groupbuy_info['curtot'] = ('第'.$groupbuy_info['qishu_curr'].'期 / 总'.$groupbuy_info['qishu_total'].'期');
        ...
        } elseif (($groupbuy_info['end_time'] >0 && $groupbuy_info['end_time'] < TIMESTAMP) && $groupbuy_info['state'] == self::GROUPBUY_STATE_NORMAL) {
