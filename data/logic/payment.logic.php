...
class paymentLogic {
    ...
    public function getRealOrderInfo($pay_sn, $member_id = null) {
        ...
        $vr_order_list = Model()->table('vr_order')->where(array('pay_sn' => $pay_sn))->select();
        if(empty($order_pay_info) and empty($vr_order_list)){
        ...
        if (!empty($vr_order_list)) {
           foreach ($vr_order_list as $val) {
             $pay_amount += ncPriceFormat(floatval($val['order_amount']) - floatval($val['rcb_amount']) - floatval($val['pd_amount']));
          }
        }
        if (!empty($order_list)) {
        ...
        $order_pay_info['vr_order_list'] = $vr_order_list;
