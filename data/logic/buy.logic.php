...
class buyLogic {
    ...
    * @param unknown $store_id价格日历过来的参数
    * @return Ambigous <multitype:unknown, multitype:unknown >
    */
   public function buyStep1($cart_id, $ifcart, $member_id, $store_id,$calendar_date) {
      //得到购买商品信息
      if ($ifcart) {
         $result = $this->getCartList($cart_id, $member_id,$calendar_date);
      } else {
         $result = $this->getGoodsList($cart_id, $member_id, $store_id,$calendar_date);
      }
    ...
    * @param str $calendar_date 价格日历参数
    */
   public function getCartList($cart_id, $member_id,$calendar_date) {
    ...
    * @param str $calendar_date 价格日历参数
    */
   public function getGoodsList($cart_id, $member_id, $store_id,$calendar_date) {
      //取得POST ID和购买数量
         $buy_items = $this->_parseItems($cart_id);
         if (empty($buy_items)) {
            return callback(false, '所购商品无效');
         }
      $goods_id = key($buy_items);//键名
      $quantity = current($buy_items);
      //价格日历POST的数据

      //商品信息[得到最新商品属性及促销信息]
      $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($goods_id, intval($quantity),$calendar_date);
      if (empty($goods_info)) {
   ...
   public function getBuyStep1Data($member_id, $data) {
      ...
      $result['store_cart_list']                 = $store_cart_list;//商品信息
      $result['store_goods_total']               = $store_goods_total;//店铺总金额
      ...
      //返回店铺可用的代金券
      $store_voucher_list           = $this->_logic_buy_1->getStoreAvailableNoLimitVoucherList($store_goods_total, $member_id);
    ...
    * @return array
    */
   public function normalBuyStep2($post, $member_id, $member_name, $member_email) {
    ...
    * @return array
    */
   public function weixinbuyStep2($post, $member_id, $member_name, $member_email) {
      ...
      try {
         ...
         //第1步 表单验证
         $this->_weixincreateOrderStep1();

         //第2步 得到购买商品信息
         $this->_weixincreateOrderStep2();
   ...
   public function checkFcode($goods_commonid, $fcode) {
      $fcode_info = Model('goods_fcode')->getGoodsFCode(array('goods_commonid' => $goods_commonid, 'fc_code' => $fcode));
      if ($fcode_info) {
         if ($fcode_info["fc_state"]) {
            return callback(false, 'F码已使用');
         } else {
            return callback(true, '', $fcode_info);
         }
      } else {
         return callback(false, 'F码错误,请重新输入');
      }
   }
   ...
   private function _createOrderStep1() {
      ...
      $input_if_vat = ('allow_vat' == $input_if_vat) ? true : false;
      ...
      $input_if_offpay = ('allow_offpay' == $input_if_offpay) ? true : false;
      ...
      if (!empty($post['voucher']) && is_array($post['voucher'])) {
         foreach ($post['voucher'] as $store_id => $voucher) {
            if (preg_match_all('/^(\d+)\|(\d+)\|([\d.]+)|(\d+)\|([\d.]+)\$/', $voucher, $matchs)) {
               if (floatval($matchs[3][0]) > 0) {
                  ...
                  $input_voucher_list[$store_id]['voucher_id']    = $matchs[4][0];
               }
            }
         }
      } elseif (!empty($post["voucher_user"])) {
         //处理外部代金券---
         $voucher_code       = $post["voucher_user"];
         $input_voucher_list = Model('buy')->up_input_voucher_list($voucher_code);
      }
      //保存数据
      ...
      $this->_order_data['order_from']            = 2 == $post['order_from'] ? 2 : 1;
   }
   /**
    * 订单生成前的表单验证与处理
    *
    */
   private function _weixincreateOrderStep1() {
      $post = $this->_post_data;
   ...
   private function _createOrderStep2() {
      ...
      if ($post['ifcart']) {
         ...
         //商品信息[得到最新商品属性及促销信息]
         $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($goods_id, intval($quantity),$calendar_array);
         if (empty($goods_info)) {
         ...
         $goods_list      = array();
//         var_dump($post);exit();
         if($post['calendar_type'] == 1){
            //普通（如门票
            $goods_info['putong_date'] = $post['start_time'];//出发时间
            $goods_info['putong_man_price'] = $post['putong_man_price'];//成人价
            $goods_info['putong_child_price'] = $post['putong_child_price'];//儿童价
            $goods_info['goods_num'] = $post['goods_num'];//数量
            $goods_info['calendar_total'] = $post['price_total'];//商品总价
            $goods_info['storage_state'] = $post['storage_state'];//库存
            $goods_info['goods_price'] = $post['price_total'];//创建订单时候的金额
            $goods_info['state'] = $post['state'];//是否能购买的状态  1 和0
            $goods_info['calendar_type'] = 1;//日历类型

            $this->_order_data['calendar_type'] =1;//后面计算总金额生成订单时候的标识
            $this->_order_data['calendar_total'] =$post['hotel_total'];//订单总价;


         }else if($post['calendar_type'] == 2){
            //酒店
            $goods_info['goods_num'] = $post['hotel_num'];//入住几晚
            $goods_info['hotel_state_time'] = $post['hotel_state_time'];//入住时间
            $goods_info['hotel_end_time'] = $post['hotel_end_time'];//离店时间
            $goods_info['storage_state'] = $post['hotel_stock'];//库存
            $goods_info['calendar_total'] = $post['hotel_total'];//订单总价
            $goods_info['state'] = $post['state'];//是否能购买的状态  1 和0
            $goods_info['calendar_type'] = 2;//算价格用
            $goods_info['goods_price'] = $post['hotel_total'];//创建订单时候的金额
            $this->_order_data['calendar_type'] =2;//后面计算总金额生成订单时候的标识
            $this->_order_data['calendar_total'] =$post['hotel_total'];//订单总价;

         }else if($post['calendar_type'] == 3){
            //高尔夫
            $goods_info['storage_state'] = $post['storage_state'];//库存
            $goods_info['calendar_total'] = $post['price_total'];//商品价格，高尔夫是单个订单，这个即为总价
            $goods_info['state'] = $post['state'];//验证是否可以购买
            $goods_info['calendar_type'] = 3;//算价格用
            $goods_info['goods_price'] = $post['price_total'];//创建订单时候的金额
            $this->_order_data['calendar_type'] =3;//后面计算总金额生成订单时候的标识
            $this->_order_data['calendar_total'] =$post['hotel_total'];//订单总价;
         }
         $goods_list[0]   = $store_cart_list[$goods_info['store_id']][0]   = $goods_info;
    ...
    *
    */
   private function _weixincreateOrderStep2() {
      $post            = $this->_post_data;
      $input_buy_items = $this->_order_data['input_buy_items'];
      ...
      if ($post['ifcart']) {
         ...
         //商品信息[得到最新商品属性及促销信息]
         $goods_info = $this->_logic_buy_1->getGoodsWeixinInfo($goods_id, intval($quantity));
         if (empty($goods_info)) {
            throw new Exception('商品已下架或不存在');
         }
   ...
   private function _createOrderStep3() {
      ...
      $calendar_type      = $this->_order_data['calendar_type'];
      ...
      if (false === $append_premiums_to_cart_list) {
    ...
    */
   private function _normalcreateOrderStep4() {
      ...
      foreach ($store_cart_list as $store_id => $goods_list) {
         ...
         $order['order_state']  = 'online' == $store_pay_type_list[$store_id] ? ORDER_STATE_NEW : ORDER_STATE_PAY;
         ...
         //存储商家发货提醒数据
         if ('offline' == $store_pay_type_list[$store_id]) {
   ...
   private function _createOrderStep4() {
      ...
      $_mk_vr_order_sn = function($member_id) {
      ...
      foreach ($store_cart_list as $store_id => $goods_list) {
         foreach ($goods_list as $k => $val) {
            if ($val['is_virtual']) {
               $has_virtual = true;
               $val['rl_data'] = base64_encode($val['calendar_date']);
               $vr_order_info = array(
                  'goods_name' => $val['goods_name'],
                  'goods_price' => $val['goods_price'],
                  'order_sn' => $_mk_vr_order_sn($val['buyer_id']),
                  'pay_sn' => $pay_sn,
                  'store_id' => $store_id,
                  'store_name' => $val['store_name'],
                  'buyer_id' => $val['buyer_id'],
                  'buyer_name' => $member_name,
                  'buyer_phone' => $this->_post_data['buyer_phone'],
                  'add_time' => TIMESTAMP,
                  'buyer_msg' => $input_pay_message[$store_id],
                  'order_state' => ORDER_STATE_NEW,
                  'order_amount' => $val['goods_total'],
                  ...
                  'commis_rate' => 200,
                  'gc_id' => $val['gc_id'],
                  'vr_indate' => $val['virtual_indate'],
                  'vr_invalid_refund' => $val['virtual_invalid_refund'],
                  'order_from' => 1,
                  'order_info' => serialize($val),
                  'payment_code' => 'online',
               );
               if (1 == $val['ifgroupbuy']) {
                  $vr_order_info['order_promotion_type'] = 1;
                  $vr_order_info['promotions_id']        = $val['groupbuy_id'];
               }
               if (1 == $_SESSION['share_shop'] and 1 == $val['isshare']) {
                  $vr_order_info['is_share'] = 1;
                  #计算佣金
                  $order_re_total_1 = $val['re_price_1_total'];
                  $order_re_total_2 = $val['re_price_2_total'];
                  $order_re_total_3 = $val['re_price_3_total'];
                  $vr_order_info['re_price_1'] = $order_re_total_1;
                  $vr_order_info['re_price_2'] = $order_re_total_2;
                  $vr_order_info['re_price_3'] = $order_re_total_3;
                  $vr_order_info['re_status']  = 0;
                  #获取上级信息
                  $wx_model = Model('share_member');
                  $wx_info  = $wx_model->where(array('member_id' => $member_id))->find();
                  $re_mid_1 = 0;
                  $re_mid_2 = 0;
                  $re_mid_3 = 0;
                  #是否是分销商自购 --暂时不考虑，一律返上三级

                  if ($wx_info['pid']) {
                     $re_info  = $wx_model->where(array('member_id' => $wx_info['pid']))->find();
                     $re_mid_1 = $re_info['member_id'];
                     if ($re_info['pid']) {
                        $re_info_2 = $wx_model->where(array('member_id' => $re_info['pid']))->find();
                        $re_mid_2  = $re_info_2['member_id'];
                        if ($re_info['pid']) {
                           $re_info_3 = $wx_model->where(array('member_id' => $re_info_2['pid']))->find();
                           $re_mid_3  = $re_info_3['member_id'];
                        } else {
                           $vr_order_info['re_price_3'] = 0;
                        }
                     } else {
                        $vr_order_info['re_price_2'] = 0;
                        $vr_order_info['re_price_3'] = 0;
                     }
                  } else {
                     $vr_order_info['re_price_1'] = 0;
                     $vr_order_info['re_price_2'] = 0;
                     $vr_order_info['re_price_3'] = 0;
                  }
                  $vr_order_info['re_mid_1'] = $re_mid_1;
                  $vr_order_info['re_mid_2'] = $re_mid_2;
                  $vr_order_info['re_mid_3'] = $re_mid_3;
               }
               if($vr_order_info['re_mid_1']<=0){
                  $vr_order_info['re_price_1'] = 0;
               }
               if($vr_order_info['re_mid_2']<=0){
                  $vr_order_info['re_price_2'] = 0;
               }
               if($vr_order_info['re_mid_3']<=0){
                  $vr_order_info['re_price_3'] = 0;
               }
               $vr_order_id = $model_vr_order->addOrder($vr_order_info);
               if (!$vr_order_id) {
                  throw new Exception('订单保存失败[vr]');
               }
               $vr_order_info['order_id'] = $vr_order_id;
               $order_list['vr'][$vr_order_id] = $vr_order_info;
               if ($val['goods_storage_alarm'] >= ($val['goods_storage'] - $val['goods_num'])) {
                  $param              = array();
                  $param['common_id'] = $val['goods_commonid'];
                  $param['sku_id']    = $val['goods_id'];
                  QueueClient::push('sendStoreMsg', array('code' => 'goods_storage_alarm', 'store_id' => $val['store_id'], 'param' => $param));
               }
               unset($goods_list[$k]);
               $store_final_order_total[$store_id] -= $val['goods_total'];
            }
         }
         if (!empty($goods_list)) {
            //取得本店优惠额度(后面用来计算每件商品实际支付金额，结算需要)
            ...
            if (1 == $_SESSION['share_shop']) {
               ...
               foreach ($goods_list as $item) {
                  if (1 == $item['is_share']) {
                     $order_re_total_1 += $item['re_price_1_total'];
                     $order_re_total_2 += $item['re_price_2_total'];
                     $order_re_total_3 += $item['re_price_3_total'];
                     $order['is_share'] = 1;
                  }
               }
               if ($order['is_share'] = 1) {
                  $order['re_price_1'] = $order_re_total_1;
                  $order['re_price_2'] = $order_re_total_2;
                  $order['re_price_3'] = $order_re_total_3;
                  $order['re_status']  = 0;
                  #获取上级信息
                  $wx_model = Model('share_member');
                  $wx_info  = $wx_model->where(array('member_id' => $member_id))->find();
                  ...
                  $order['re_mid_1'] = $re_mid_1;
                  $order['re_mid_2'] = $re_mid_2;
                  $order['re_mid_3'] = $re_mid_3;
               }
            }
            if($order['re_mid_1']<=0){
               $order['re_price_1'] = 0;
            ...
            $order['buyer_email']  = $member_email;
            $order['add_time']     = TIMESTAMP;
            $order['payment_code'] = $has_virtual ? 'online' :  $store_pay_type_list[$store_id];
            $order['order_state']  = ($has_virtual or 'online' == $store_pay_type_list[$store_id]) ? ORDER_STATE_NEW : ORDER_STATE_PAY;
            $order['order_amount'] = $store_final_order_total[$store_id];
            $order['shipping_fee'] = $store_freight_total[$store_id];
            $order['goods_amount'] = $order['order_amount'] - $order['shipping_fee'];
            $order['order_from']   = $order_from;
            $code                  = cookie('code');
            $order['code']         = $code ?: '';
            $order['is_share']     = $this->_post_data['is_share'];
            //中山小修改货到付款bug>>>
            if ("" == $order['payment_code']) {
               $order['payment_code'] = "offline";
            }
            //中山小修改货到付款bug<<<
            // echo 'INSERT INTO '.DBPRE.'order(`'.implode("`,`", array_keys($order)).'`) values(\''.implode("','", $order).'\')';exit;
            $order_id = $model_order->addOrder($order);
            if (!$order_id) {
               throw new Exception('订单保存失败[未生成订单数据]');
            }
            $order['order_id']     = $order_id;
            $order_list['real'][$order_id] = $order;
            ...
            foreach ($goods_list as $goods_info) {
               ...
               if (1 == $_SESSION['share_shop'] and 1 == $goods_info['is_share']) {
                  #佣金
                  if ($re_mid_1) {
                     $order_goods[$i]['re_price_1'] = $goods_info['re_price_1_total'];
                  } else {
                     $order_goods[$i]['re_price_1'] = 0;
                  }
                  if ($re_mid_2) {
                     $order_goods[$i]['re_price_2'] = $goods_info['re_price_2_total'];
                  } else {
                     $order_goods[$i]['re_price_2'] = 0;
                  }
                  if ($re_mid_3) {
                     $order_goods[$i]['re_price_3'] = $goods_info['re_price_3_total'];
                  } else {
                     $order_goods[$i]['re_price_3'] = 0;
                  }
               } else {
                  $order_goods[$i]['re_price_1'] = 0;
                  $order_goods[$i]['re_price_2'] = 0;
               ...
               if (!intval($goods_info['bl_id'])) {
                  ...
                  foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                     ...
                     //计算商品实际支付金额(goods_price减去分摊优惠金额后的值)
                     $goods_total = $bl_goods_info['bl_goods_price'] * $goods_info['goods_num'];//总额
                     //计算本件商品优惠金额
                     $promotion_value                    = floor($goods_total * ($promotion_rate));//优惠金额
                     $order_goods[$i]['goods_pay_price'] = $goods_total - $promotion_value;//实际支付金额
                     $promotion_sum += $promotion_value;
                     $i++;
   ...
   //         if($calendar_type == 3){//高尔夫
   //            $golf_insert['']
   //            $insert_golf = $model_order->addOrderGolf($golf_insert);
   //            if (!$insert_golf) {
   //               throw new Exception('订单保存失败[未生成商品数据]');
   //            }
   //         }
   ...
   private function _createOrderStep5() {
      ...
      if ('' == $buyer_info['member_paypwd'] || md5($this->_post_data['password']) != $buyer_info['member_paypwd']) {
         return;
      }

      //使用充值卡支付
      if (!empty($this->_post_data['rcb_pay'])) {
         if (isset($this->_order_data['order_list']['vr']) and !empty($this->_order_data['order_list']['vr'])) {
            $vr_logic = logic('buy_virtual');
            foreach ($this->_order_data['order_list']['vr'] as $key => $order_info) {
               $order_info = $vr_logic->_rcbPay($order_info, $this->_post_data, $buyer_info);
               $this->_order_data['order_list']['vr'][$key] = $order_info;
            }
         }
         if (!empty($this->_order_data['order_list']['real'])) {
            $order_list = $this->_logic_buy_1->rcbPay($this->_order_data['order_list']['real'], $this->_post_data, $buyer_info);
         }
      }
      ...
      if (!empty($this->_post_data['pd_pay'])) {
         if (isset($this->_order_data['order_list']['vr']) and !empty($this->_order_data['order_list']['vr'])) {
            $vr_logic = logic('buy_virtual');
            foreach ($this->_order_data['order_list']['vr'] as $key => $order_info) {
               $order_info = $vr_logic->pdPay($order_info, $this->_post_data, $buyer_info);
               $this->_order_data['order_list']['vr'][$key] = $order_info;
            }
         }
         $order_list = $order_list ? $order_list : $this->_order_data['order_list']['real'];
         if (!empty($order_list)) {
            $this->_logic_buy_1->pdPay($order_list, $this->_post_data, $buyer_info);
         }
      }
   }
   ...
   private function _createOrderStep6() {
      ...
      //变更库存和销量
      if($this->_post_data['is_share'] == 1){
        $model_goods = Model('goods');
        foreach($goods_buy_quantity as $goods_id => $goods_num){
          $data = array();
          $data['share_stock'] = array('exp','share_stock-'.$goods_num);
          $data['goods_salenum'] = array('exp','goods_salenum+'.$goods_num);
      ...
      if (!empty($input_voucher_list) && is_array($input_voucher_list)) {
         ...
         if (!empty($input_voucher_list)) {
            $new_order_list = array_values($order_list['real']);
            $store_id       = $new_order_list[0]["store_id"];
            $order_id       = $new_order_list[0]["order_id"];
            $voucher_id     = $input_voucher_list[$store_id]["voucher_id"];
            Model('buy')->up_voucher_order_id($order_id, $voucher_id);
         }
      }
   ...
   private function _getGoodsList($cart_list) {
      ...
      foreach ($cart_list as $key => $cart) {
         ...
         if (!intval($cart['bl_id'])) {
            ...
            if (1 == $cart['is_share']) {
               $goods_list[$i]['goods_price'] = $cart['share_price'];
            } else {
            ...
            $goods_list[$i]['is_virtual']    = $cart['is_virtual'];
            $goods_list[$i]['calendar_date'] = $cart['calendar_date'];
            $goods_list[$i]['calendar_type'] = $cart['calendar_type'];
            $goods_list[$i]['virtual_indate'] = $cart['virtual_indate'];
            $goods_list[$i]['virtual_invalid_refund'] = $cart['virtual_invalid_refund'];
            $i++;
         } else {
            //如果是优惠套装商品
