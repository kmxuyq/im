...
class queueLogic {
    ...
    public function createOrderUpdateStorage($goods_buy_quantity) {
        if($goods_buy_quantity['calendar_type'] ==4){
            $updata['storage'] = array('exp','storage-'.$goods_buy_quantity['quantity']);
            $result = Model()->table('cart_operate_date')->where("goods_id = {$goods_buy_quantity['goods_id']} AND date = '{$goods_buy_quantity['ticket_date']}'")->update($updata);
        }else{
            $model_goods = Model('goods');
            foreach($goods_buy_quantity as $goods_id => $goods_num){
                $data = array();
                $data['goods_storage'] = array('exp','goods_storage-'.$goods_num);
