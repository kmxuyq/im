...
//        }
    ...
    public function buyStep3($post, $member_id) {
        ...
        //应付总金额计算  
        $pay_total=decrypt($_POST['goods_total']);//总价
       // $pay_total = $goods_info['goods_price'] * $goods_info['quantity'];
        ...
        //-----新增HOME---------
        $input['goods_name'] =decrypt($post['goods_name']);
        $input['goods_price'] = decrypt($post['goods_price']);
       // $input['goods_total'] = decrypt($post['goods_total']);
        $input['get_info'] = decrypt($post['get_info']);//订单详细
        //--------------
        $goods_info['goods_name']=$input['goods_name'];//订单名称
        $goods_info['goods_price']=$input['goods_price'];//单价
        //-------新增END-------------
        try {
     ...
     * @param  $post
     */
    private function up_stock($get_info){
    	$post=json_decode($get_info,true);
    	//print_r($post);
    	$type_id=$post["type_id"];
    	//线路门票
    	if($type_id=='40'||$type_id=='41'||$type_id=='42'||$type_id=='43'){
	    	$where_stock=array('commonid'=>$post['commonid'],'date'=>substr($post["date"],0,7),'package'=>$post["package"]);
    		...
    		$date=$post["date"];
    		$golf_minute=$post["golf_minute"];
    		$stock='2';//2为预订锁定状态，支付成功后为0
