		...
		switch ($this->style) {
			...
			case '6':
				if($this->total_page <=0){
					...
					$html_page .= '<ul><li class="bb-input-fy widthall">';
					if ($this->getNowPage() <= 1){
						$html_page .= '<input type="button" value="首页">';
						$html_page .= '<input type="button" value="上一页">';
					}else {
						$html_page .= '<input type="button" value="首页"  onclick="location.href=\''.$this->page_url.'1\'" >';
						$html_page .= '<input type="button" value="上一页"  onclick="location.href=\''.$this->page_url.($this->getNowPage()-1).'\'" >';
					}
					$html_page .= '<input type="text" value="'.$this->getNowPage().'/'.$this->getTotalPage().'" disabled="true" 　readonly="true">';
					if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
						$html_page .= '<input type="button" value="下一页">';
						$html_page .= '<input type="button" value="末页">';
					}else {
						$html_page .= '<input type="button" value="下一页"  onclick="location.href=\''.$this->page_url.($this->getNowPage()+1).'\'" >';
						$html_page .= '<input type="button" value="末页"  onclick="location.href=\''.$this->page_url.($this->getTotalPage()).'\'" >';
					}
					$html_page .= '</ul>';
				}
				break;
