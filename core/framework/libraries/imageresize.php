...
class ImageResize{
	...
	/**
	 *
	 * @param $localimg源图片文件路径
	 * @param $remoteimg目标图片路径
	 * @param $x最大宽度 $x为0则自动计算宽度
	 * @param $y最大高度  $y为0则自动计算高度
	 * $newimage=new ImageResize();
	 * $newimage->resize("11.jpg","1_lit.jpg",500,400);
	 * echo $newimage->GetLastError();
	 */
	function resize($localimg, $remoteimg, $x, $y,$quality=90) {
		if($y==0){
			$arry=getimagesize($localimg);
			$width=$arry[0];
			$height=$arry[1];
			$bl=$height/$width;
			$y=$bl*$x;
		...
		return $this->_resize($x,$y,$quality);
	}
	/**
	 * 检测当前环境是否支持GD
	 */
	 ...
	 * @return boolean
	 */
	private function _resize($x,$y,$quality){
		if(!$this->localinfo){
		...
		if(!imagejpeg($dstim, $this->remoteimage,$quality)){
			if(!imagepng($dstim,$this->remoteimage,$quality)){
