 ...
 *订单佣金自动结算
 */
function auto_apply_cmms($delay = 7)
{
    auto_apply_vr_cmms($delay);
    auto_apply_real_cmms($delay);
}

function auto_apply_vr_cmms($delay)
{
    auto_apply_order_cmms('vr_order', $delay);
}

function auto_apply_real_cmms($delay)
{
    auto_apply_order_cmms('order', $delay);
}

function auto_apply_order_cmms($table, $delay)
{
    $start_time = strtotime('-' . $delay . ' days');
    $sql        = "SELECT * FROM " . DBPRE . "{$table} WHERE is_share=1 AND re_status=1 AND order_state=40 AND finnshed_time<={$start_time}";
    ...
    if (is_array($list) and !empty($list)) {
        ...
        foreach ($list as $key => $item) {
            ...
            $re_mid    = 0;
            for ($i = 1; $i < 4; $i++) {
                $re_mid    = $item['re_mid_' . $i];
                $re_amount = $item['re_price_' . $i];
                if ($re_mid and $re_amount) {
                    $member_info = $share_model->where(array('member_id' => $re_mid, 'store_id' => $item['store_id']))->find();
                    $share_model->where(array('member_id' => $re_mid, 'store_id' => $item['store_id']))->update(array(
                        'credits'        => array('exp', 'credits+' . $re_amount),
                        'frozen_credits' => array('exp', 'credits-' . $re_amount),
                    ));
                }
            }
            Model()->table($table)->where(array('order_id' => $item['order_id']))->update(array('re_status' => 2, 're_time' => TIMESTAMP));
        }
    }
}
...
{
    //
    $url   = "http://www.myqhd.com.cn/myqhd.php?act=myqhd&op=areaInfo&ip=" . $ip;
...
function checkSeccode($nchash, $value)
{
    ...
    $return                                     = strtoupper($value) == $checkvalue && $checkidhash == $nchash;
    if (!$return) {
...
function menu($str, $id)
{
    ...
    $texta    = "滴滴打车";
    if ($str == $activity) {
        return urlShopWAP('search', 'product_list', array('type' => 'active', 'wx' => 0, 'cate_id' => $id));
    } elseif ($str == $magic) {
        return urlShopWAP('search', 'product_list', array('type' => 'magic', 'cate_id' => $id));
    } elseif ($str == $applay) {
        return '#';
    } elseif ($str == $texta) {
        return urlShopWAP('search', 'product_list', array('type' => 'appaly', 'wx' => 0, 'cate_id' => $id));
    } else {
        return urlShopWAP('search', 'product_list', array('cate_id' => $id));
    }
}
function menu_1($str, $id)
{
    $applay = "免费申领";
    if ($str == $applay) {
        return urlShopWAP('search', 'product_list', array('type' => 'appaly', 'wx' => 0, 'cate_id' => $id));
...
function getIp()
{
    if (@$_SERVER['HTTP_CLIENT_IP'] && 'unknown' != $_SERVER['HTTP_CLIENT_IP']) {
        $ip = $_SERVER['HTTP_CLIENT_IP'];
    } elseif (@$_SERVER['HTTP_X_FORWARDED_FOR'] && 'unknown' != $_SERVER['HTTP_X_FORWARDED_FOR']) {
...
function Model($model = null, $base_path = null)
{
    ...
    $base_path  = null == $base_path ? BASE_DATA_PATH : $base_path;
...
//     echo $file_name."<br>  11   <br>".$class_name;
    if (!file_exists($file_name)) {
        return $_cache[$cache_key] = new Model($model);
    } else {
        require_once $file_name;
 ...
 * @return string
 */
function getMemberAvatar($member_avatar, $member_time = '')
{
    if (empty($member_avatar)) {
        return UPLOAD_SITE_URL . DS . ATTACH_COMMON . DS . C('default_user_portrait');
    } else {
        //如果头像是微信头像则直接返回地址
        if (strpos($member_avatar, 'http://') === 0) {
            return $member_avatar;
        } elseif (strpos($member_avatar, '_weixin') > 0) {
            $member_time_path = '';
            if (!empty($member_time)) {
                $member_time_path = date('Y/m/d/', $member_time);
            }
            $filename = '/data/weixin/head/' . $member_time_path . $member_avatar;
            return $filename;
        } else {
            if (file_exists(BASE_UPLOAD_PATH . DS . ATTACH_AVATAR . DS . $member_avatar)) {
                return '/data/upload' . DS . ATTACH_AVATAR . DS . $member_avatar;
            } else {
                return UPLOAD_SITE_URL . DS . ATTACH_COMMON . DS . C('default_user_portrait');
            }
        }
...
{
    $member = Model()->table('member')->field('member_avatar')->where(array('member_id' => $id))->find();
    if (strpos($member['member_avatar'], 'avatar_') === 0) {
        return '/data/upload/' . ATTACH_AVATAR . '/avatar_' . $id . '.jpg';
    } else if (strpos($member['member_avatar'], '_weixin') > 0) {
        $filename = '/data/weixin/head/' . $member['member_avatar'];
        return $filename;

    } else if (strpos($member['member_avatar'], 'http://') === 0) {
        return $member['member_avatar'];

    } else {
        return '/data/upload/' . ATTACH_COMMON . DS . C('default_user_portrait');
    }
}
/**
 * 取得店铺标志
...
function getMbSpecialImageUrl($image_name = '')
{
    ...
    if (('' == UPLOAD_SITE_URL or strpos(C('base_site_url'), UPLOAD_SITE_URL) === 0) and !is_file(BASE_UPLOAD_PATH . $image_path) or @file_get_contents(UPLOAD_SITE_URL . $image_path) === false) {
        return UPLOAD_SITE_URL . '/' . defaultGoodsImage('240');
    }
    return UPLOAD_SITE_URL . $image_path;
}
    ...
    if (true == $ispage) {
        if ($delay_eachnum < $page_eachnum) {
            ...
            if ($page_totlepage > $page_nowpage) {
                ...
                if (false == $hasmore && $page_eachnum % $delay_eachnum > 0) {
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $mode = 'append' == $mode ? FILE_APPEND : null;
...
{
    if (null === $key || !C('cache_open') || !is_array($data)) {
        return;
    }
...
function getquery_active($query_active)
{
    //查询列表
    $condition = array();
    if (trim($query_active['goods_name'])) {
        $goods_name = array('like', '%' . trim($query_active['goods_name']) . '%');
        $goods      = Model()->table('goods')->field('*')->where(array('goods_name' => $goods_name))->select();
        foreach ((array) $goods as $k => $azg) {
            $condition['goods_id'] = $goods[$k]['goods_id'];
        }
    }
    $select_state                       = intval($query_active['select_state']);
    $condition['az_active_chick_state'] = $select_state;
    $if_start_date                      = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $query_active['query_start_date']);
    $if_end_date                        = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $query_active['query_end_date']);
    $start_unixtime                     = $if_start_date ? strtotime($query_active['query_start_date']) : null;
    $end_unixtime                       = $if_end_date ? strtotime($query_active['query_end_date']) : null;

    if ($start_unixtime || $end_unixtime) {
        $condition['az_active_addtime'] = array('time', array($start_unixtime, $end_unixtime));
    }
    if ($query_active['mob_phone']) {
        $mob_phone  = $query_active['mob_phone'];
        $_condition = array('mob_phone' => $mob_phone);
        $address    = Model()->table('address')->field('*')->where($_condition)->select();
        foreach ((array) $address as $k => $azg) {
            $condition['address_id'] = $azg['address_id'];
        }
    }
    if (trim($query_active['member_name'])) {
        $true_name  = array('like', '%' . trim($query_active['member_name']) . '%');
        $_condition = array('true_name' => $true_name);
        $address    = Model()->table('address')->field('*')->where($_condition)->select();
        foreach ((array) $address as $k => $azg) {
            $condition['address_id'] = $azg['address_id'];
        }
 ...
 * @return string
 */
function getChat($layout, $mobile)
{
    ...
    return Chat::getChatHtml($layout, $mobile);
}
 ...
 * @return string
 */
function urlWapShop($act = '', $op = '', $args = array())
{
    return url($act, $op, $args, false, WAP_SHOP_SITE_URL);
}
...
/**
 * @param $v提示信息
 * @param $gourl  -1提并交返回   0提示并关闭    如果为网址则返回指定网址
 */
function callmsg($v, $gourl)
{
    switch ($gourl) {
        case "-1":
            echo "<meta charset='utf-8'/><script Language=javascript>alert('" . $v . "');history.go(-1);</script>";
            break;
        case "0":
            echo "<meta charset='utf-8'/><script Language=javascript>alert('" . $v . "');</script>";
            echo "<Script language=Javascript>window.opener=null;window.close();</Script>";
            break;
        case "1":
            echo "<meta charset='utf-8'/><script Language=javascript>alert('" . $v . "');window.history.back();</script>";
            break;
        default:
            echo "<meta charset='utf-8'/><script Language=javascript>alert('" . $v . "');window.location.href='" . $gourl . "';</script>";
            break;
    }
    exit;
}
...
/**
 * 产品名称特殊字符替换,默认解密
 * @param unknown_type $str字符串
 * @param unknown_type $mode模式，en为加密，de为解密
 */
function goods_name_encode($str, $mode = 'de')
{
    if ('en' == $mode) {
        $str_new = str_replace(array('+'), array('jia'), $str);
    } else {
        $str_new = str_replace(array('jia'), array('+'), $str);
    }
    return $str_new;
}
