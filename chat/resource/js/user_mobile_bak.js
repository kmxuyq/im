...
function alert_user_msg(u_id) {
    ...
    if (user_list[u_id]['new_msg'] > 0 && $(".customerinforlist span[u_id='" + u_id + "']").length > 0) {
        ...
        var arr = msg.split('goods_id@');
        if (typeof arr[1] !== "undefined") {
            var new_msg = '产品信息';
        } else if (msg.substring(1, 4) == 'img') {
            var new_msg = '图片信息';
        } else {
        ...
        $("#goods_url" + u_id + "").attr('href', '/chat/index.php?act=web_chat&op=chat_online&goods_id=' + msg.chat_goods.goods_id + '&uid=' + u_id + '');
        $("#recent_t_msg" + u_id + "").text(new_msg);
        $(".contacts").addClass("block");
        // $('.customerlist').addClass("block");
    } else {
