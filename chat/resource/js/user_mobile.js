...
var dialog_show = $('#dialog_show').val(); //对话框是否打开
var user_show = getQueryString('uid'); //当前选择的会员
var msg_max = 20; //消息数
var time_max = 10; //定时(分钟)刷新防止登录超时退出,为0时关闭
var chat_audio = 1; //消息提醒声音开关
var chat_goods_id = getQueryString('goods_id');
var CHAT_SITE_URL = '/chat';
var RESOURCE_SITE_URL = '/data/resource';
var audio_info = '<object width="1" height="1" style="position: absolute; left: -1px;" id="msg_audio" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">' + '<param value="' + CHAT_SITE_URL + '/resource/audio1.swf" name="movie"><param value="always" name="allowscriptaccess">' + '<embed width="1" height="1" allowscriptaccess="always" src="' + CHAT_SITE_URL + '/resource/audio.swf" name="msg_audio"></object>';
...
$(function() {
    ...
    if (getQueryString('op') == 'messagelist') {
        ...
        $.ajax({
            ...
            success: function(u_list) {
                for (var u_id in u_list) {
                    ...
                    s_html = '';
                    s_html += '<li class="information"> <div class="informlist-li"> <div class="icon"> <div class="customer_touxiang"><img src="' + user_info['avatar'] + '"></div> <div class="customerinforlist"> <p><span class="inforlistname">怡美活动</span><span class="inforlistdate">16/02/23</span></p> <p><span class="inforlistintor">哇~懂你如我，简直是小助手！</span><span class="informessge">9</span></p> </div> </div> <div class="btn">删除</div> </div> </li>';
                    $('#s_list').html(s_html);
                }
            }
        });
    ...
    if (user['seller_id'] != '') {
        ...
        var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>' + '<div id="chat_user_list" class="chat-list-content"><div><dl id="chat_user_sellers"><dt onclick="chat_show_user_list(\'sellers\');">' + '<span class="show"></span>商家客服</dt><dd id="chat_sellers" style="display: none;"></dd></dl>' + '<dl id="chat_user_recent"><dt onclick="chat_show_user_list(\'recent\');"><span class="show"></span>最近联系人</dt><dd id="chat_recent" style="display: none;"></dd></dl></div></div>' + '</div></div>';
        var ajaxurl = CHAT_SITE_URL + '/index.php?act=web_chat&op=get_seller_list&n=99&f_id=' + user['u_id'];
        $.ajax({
            type: "GET",
            url: ajaxurl,
            ...
            success: function(u_list) {
                ...
                $(document.body).delegate('#im', 'click', function() {
                    chat_show_list();
                });
            }
        });
    ...
    if (user['u_id'] != '') {
        web_info['chat_goods_html'] = '';
        web_info['html_title'] = $(document).attr('title');
        web_info['msg_dialog'] = '<div id="new_msg_dialog" class="msg-windows"><div class="user-tab-bar"><ul class="user-list" id="user_list"></ul></div>' + '<div class="msg-dialog"><div class="dialog-body">' + '<div id="msg_list" class="msg-contnet"><div id="user_msg_list"></div></div>' + '<div class="msg-input-box"><div class="msg-input-title"><a id="chat_show_smilies" href="javascript:void(0)" class="chat_smiles">表情</a>' + '<a href="javascript:void(0);" onclick="send_img()">图片</a>' + '<span class="title">输入聊天信息</span><span class="chat-log-btn off" onclick="show_chat_log();">聊天记录<i></i></span></div>' + '<form id="msg_form"><textarea name="send_message" id="send_message" class="textarea" onkeyup="send_keyup(event);" onfocus="send_focus();" ></textarea>' + '<div class="msg-bottom"><div id="msg_count"></div><a href="JavaScript:void(0);" onclick="send_msg();" class="msg-button"><i></i>发送消息</a><div id="send_alert"></div></div></form></div></div>' + '<div id="dialog_chat_right" class="dialog-chat-right"></div><div id="dialog_chat_log" class="dialog_chat_log"></div></div>' + '<div id="dialog_clear" class="dialog_clear"></div></div>';
        var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>' + '<div id="chat_user_list" class="chat-list-content"><div><dl id="chat_user_friends"><dt onclick="chat_show_user_list(\'friends\');">' + '<span class="show"></span>我的好友</dt><dd id="chat_friends" style="display: none;"></dd></dl>' + '<dl id="chat_user_recent"><dt onclick="chat_show_user_list(\'recent\');"><span class="show"></span>最近联系人</dt><dd id="chat_recent" style="display: none;"></dd></dl></div></div>' + '</div></div>';
        ...
        $.ajax({
            ...
            success: function(u_list) {
                ...
                if (user_list[getQueryString('uid')]) {
                    $('#header_info').text('对' + user_list[getQueryString('uid')].u_name + '..对话中')
                }
                setTimeout("getconnect()", 1000);
                $("#web_chat_dialog").prepend(chat_user_list);
                $('#chat_user_list').perfectScrollbar();
        ...
        if (chat_goods_id != '' && chat_goods_id != 10086) {
            ...
            $.ajax({
                ...
                success: function(goods) {
                    if (typeof goods['goods_id'] !== "undefined") {
                        ...
                        var goods_html = '';
                        goods_html += '<div class="product-img"> <img src="' + goods['pic'] + '" alt="' + goods['goods_name'] + '" style="width:100%;height: inherit;"/> </div> <dl class="product-info"> <dt>' + goods['goods_name'].substring(0, 12) + '</dt> <dd class="product-price"><em>￥</em><span class="price__new">' + goods['goods_promotion_price'] + '</span><span class="price__old">￥' + goods['goods_marketprice'] + '</span></dd> <dd class="product-btn"> <a href="javascript:void(0);" onclick="send_msg(\'goods_id@' + goods['goods_id'] + '\')">发送产品链接</a> </dd> </dl>';
...
function msg_dialog_close(id) {
    if (dialog_show == 1) $("#" + id).hide("slide", {
        direction: 'right'
    }, 300);
    dialog_show = 0;
...
function send_msg(product_id) { //发消息
    if (user_show < 1) {
    ...
    msg['goods_id'] = chat_goods_id;
    msg['t_id'] = user_show;
    msg['t_name'] = user_list[user_show]['u_name'];
    if (product_id) {
        var ajaxurl = CHAT_SITE_URL + '/index.php?act=web_chat&op=get_goods_info&goods_id=' + product_id;
        $.ajax({
            type: "GET",
            url: ajaxurl,
            dataType: "jsonp",
    ...
    if (msg['t_msg'].length > 255) {
        ...
        layer.msg('一次最多只能发送255字');
        return false;
    }
    if (connect < 1) {
        $("#send_alert").html('处于离线状态,稍后再试');
        layer.msg('处于离线状态,稍后再试');
        return false;
    }
    $.ajax({
        type: "POST",
...
function get_chat_msg(t) {
    ...
    var ajaxurl = CHAT_SITE_URL + '/index.php?act=web_chat&op=get_chat_log&page=30&f_id=' + user['u_id'] + '&goods_id=' + goods_id + '&t_id=' + chat_log['u_id'] + '&t=' + chat_log['time_from'];
...
function show_msg(u_id) { //显示会员的消息
    ...
    if (user_show != u_id) {
        ...
        if (typeof user_info['s_name'] !== "undefined" && user_info['s_name'] !== "") {
            ...
            store_html = '<a target="_blank" href="' + SHOP_SITE_URL + '/index.php?act=show_store&op=index&store_id=' + user_info['s_id'] + '">' + user_info['s_name'] + '</a>';
            u_name = '客服：' + u_name;
        }
...
function show_f_msg(msg) { //显示收到的消息
    ...
    var text_append = '';
    var obj_msg = obj.find("div[select_user_msg='" + u_id + "']");
    var obj_msg = $('#msg_list');
    text_append += '<div class="chat-left-item"> <div class="chat-time">' + msg['add_time'] + '</div> <div class="chat-info"> <div class="head-pic"> <img src="' + user_info['avatar'] + '"/> </div> <div class="content-info"> <div class="nick-name">' + msg['f_name'] + '</div> <div class="content"> <i class="jiantou"></i> ' + update_chat_msg(msg['t_msg']) + ' </div> </div> </div> </div>';
    // text_append = '</div>';
...
function show_t_msg(msg) { //显示发出的消息
    ...
    var obj_msg = obj.find("div[select_user_msg='" + u_id + "']");
    var obj_msg = $('#msg_list');
    text_append += '<div class="chat-right-item"> <div class="chat-time">' + msg['add_time'] + '</div> <div class="chat-info"> <div class="head-pic"> <img src="' + user_info['avatar'] + '"/> </div> <div class="content-info"> <div class="nick-name">' + msg['f_name'] + '</div> <div class="content"> <i class="jiantou"></i> ' + update_chat_msg(msg['t_msg']) + ' </div> </div> </div> </div>';
    obj_msg.append(text_append);
    get_goods_info(msg);
    var n = obj_msg.find("div[m_id]").size();
    if (n >= msg_max && n % msg_max == 1) {
...
function alert_user_msg(u_id) {
    var msg = '';
    if (user_list[u_id]['new_msg'] > 0 && $(".customerinforlist span[u_id='" + u_id + "']").length > 0) {
        obj.find("em[unread_id='" + u_id + "']").addClass("unread");
        obj.find("em[unread_id='" + u_id + "']").html(user_list[u_id]['new_msg']);
        $(".customerinforlist span[u_id='" + u_id + "']").addClass("informessge");
        $(".customerinforlist span[u_id='" + u_id + "']").css("display", 'block');
        $(".customerinforlist span[u_id='" + u_id + "']").text(user_list[u_id]['new_msg']);
        var msg = msg_list[u_id][msg_list[u_id].length - 1].t_msg;
        var arr = msg.split('@');
        var msg_img = /\.[^\.]+$/.exec(msg);
        if (arr[0] == "img") {
            var new_msg = '图片信息';
        } else if (arr[0] == "goods_id") {
            var new_msg = '产品信息';
        } else {
            var new_msg = msg_list[u_id][msg_list[u_id].length - 1].t_msg;
        }
        new_msg = new_msg.substring(0, 15);
        var msg = msg_list[u_id][msg_list[u_id].length - 1];
        $("#t_msg" + u_id + "").text(new_msg);
        var unread = user_list[u_id]['new_msg'];
        $("#goods_url" + u_id + "").attr('href', '/chat/index.php?act=web_chat&op=chat_online&goods_id=' + msg.chat_goods.goods_id + '&uid=' + u_id + '&unread=' + unread + '&is_chat=1');
        $("#recent_t_msg" + u_id + "").text(new_msg);
        $(".contacts").addClass("block");
    } else {
        obj.find("em[unread_id='" + u_id + "']").html("");
        obj.find("em[unread_id='" + u_id + "']").removeClass("unread");
        $(".customerinforlist span[u_id='" + u_id + "']").removeClass("msg");
        $(".customerinforlist span[u_id='" + u_id + "']").css("display", 'none');
        var obj_recent = $("#chat_recent");
        obj_recent.before('<div id="chat_recent">');
        ...
        var msg = msg_list[u_id][msg_list[u_id].length - 1];
        if (getQueryString('op') == 'messagelist' && msg.chat_goods.goods_id != "undefined") {
            if (user_list[u_id]['u_id'] != null) {
                obj_recent.before('<li class="information"> <div class="informlist-li"> <a id="goods_url' + u_id + '" href="/chat/index.php?act=web_chat&op=chat_online&goods_id=' + msg.chat_goods.goods_id + '&uid=' + u_id + '&unread=1&is_chat=1"><div class="icon"> <div class="customer_touxiang"> <img src="' + user_list[u_id]['avatar'] + '"/> </div> <div class="customerinforlist"> <p> <span class="inforlistname"> ' + user_list[u_id]['u_name'] + ' </span> <span class="inforlistdate"> ' + new Date().Format("yy/MM/dd") + ' </span> </p> <p> <span id="t_msg' + u_id + '" class="inforlistintor"> ' + msg.t_msg + ' </span> <span u_id="' + u_id + '" class="informessge"> ' + user_list[u_id]['new_msg'] + ' </span> </p> </div> </div> </a><div class="btn" onclick="del_user_msg(' + u_id + ')"> 删除 </div> </div> </li>');
            }
        }
        obj_recent.before('</div>');
        moveDeleteFn()
        $(".contacts").addClass("block");
    }
}
...
function play_audio() { //提示声音
    if (chat_audio === 1) {
        var audio = new Audio(CHAT_SITE_URL + '/resource/msg.mp3');
        if (getQueryString('op') == 'messagelist') {
            audio.play();
        }
...
function moveDeleteFn(width) {
    var _self = this;
    var startX; //触摸位置
    var moveX; //滑动时的位置
    var X = 0;
    var endX; //移动距离
    $(".informlist-li").on("touchstart", function(e) {
        startX = event.targetTouches[0].pageX;
    });
    $(".informlist-li").on("touchmove", function(e) {
        endX = event.targetTouches[0].pageX;
    });
    $(".informlist-li").on("touchend", function(e) {
        X = endX - startX;
        if (X >= 0) {
            $(this).removeClass('translate').addClass('init_translate');
        } else if (X < 0) {
            inittranslate()
            $(this).removeClass('init_translate').addClass('translate');
        }
    });
}

function inittranslate() {
    $('.informlist-li').addClass('init_translate')
}

function del_user_msg(u_id) {
    show_msg(u_id);
    var new_n = 0;
    new_msg = new_msg - user_list[u_id]['new_msg'];
    new_interval = setInterval(function() {
        new_n++;
        if (new_msg == 0) {
            $(document).attr('title', web_info['html_title']);
    ...
    var ajaxurl = CHAT_SITE_URL + '/index.php?act=web_chat&op=del_user_msg&f_id=' + u_id;
    $.ajax({
        type: "GET",
        url: ajaxurl,
...
function update_chat_msg(msg) {
    ...
    if (img[0] == "img") {
        ...
        var msg = '<div style="padding-bottom:15px;width:' + width + 'rem;height:' + height + 'rem;"><img src="' + img[3] + '" style="display: block;width:100%;height:100%;" /><div>';
    } else if (img[0] == "goods_id") {
        var goods = goods_list[goods_id];
        var msg = '<a href="/wap_shop/index.php?act=goods&op=index&goods_id=' + goods_id + '"><div class="content-product clearfix"> <div class="product-img"> <img style="width:100%;height: inherit;" src="' + goods['pic'] + '" alt="' + goods['goods_name'] + '"> </div> <dl class="product-info"> <dt>' + goods['goods_name'] + '</dt> <dd class="product-price"><em>￥</em><span class="price__new">' + goods['goods_promotion_price'] + '</span><span class="price__old">￥' + goods['goods_marketprice'] + '</span></dd> </dl> </div></a>';
        msg = msg.replace(re, msg);
    } else {
        var msg = '<p>' + msg + '</p>';
    }
    return msg;
...
function update_chat_msg_smiles(msg) {
    if (typeof smilies_array !== "undefined") {
        msg = '' + msg;
        for (var i in smilies_array[1]) {
            var s = smilies_array[1][i];
...
function update_friends() {
    ...
    for (var u_id in friend_list) {
        if (obj_friend.parent().find("dd[u_id='" + u_id + "']").size() == 0) {
            obj_friend.before('<li class="information"> <div class="informlist-li"> <div class="icon"> <div class="customer_touxiang"> <img src="' + user_list[u_id]['avatar'] + '"/> </div> <div class="customerinforlist"> <p> <span class="inforlistname"> ' + user_list[u_id]['u_name'] + ' </span> <span class="inforlistdate"> ' + recent_list[u_id]['time'] + ' </span> </p> <p> <span class="inforlistintor"> 哇~懂你如我，简直是小助手！ </span> <span class="informessge"> 9 </span> </p> </div> </div> <div class="btn"> 删除 </div> </div> </li>');
        }
    }
    obj_friend.remove();
...
function update_recent() {
    ...
    for (var u_id in recent_list) {
        if (recent_list[u_id]['u_id'] != null) {
            var msg = recent_list[u_id]['new_msg'];
            var arr = msg.split('@');
            if (arr[0] == "img") {
                var new_msg = '图片信息';
            } else if (arr[0] == "goods_id") {
                ...
                var new_msg = recent_list[u_id]['new_msg'];
            }
            html += '<li class="information"> <div class="informlist-li"> <a id="goods_url' + u_id + '" href="/chat/index.php?act=web_chat&op=chat_online&goods_id=' + recent_list[u_id]['goods_id'] + '&uid=' + u_id + '&is_chat=1"><div class="icon"> <div class="customer_touxiang"> <img src="' + user_list[u_id]['avatar'] + '"/> </div> <div class="customerinforlist"> <p> <span class="inforlistname"> ' + user_list[u_id]['u_name'] + ' </span> <span class="inforlistdate"> ' + recent_list[u_id]['time'] + ' </span> </p> <p> <span id="t_msg' + u_id + '" class="inforlistintor"> ' + new_msg + ' </span> <span u_id="' + u_id + '" class="" style="display:none;"> 9 </span> </p> </div> </div> </a><div class="btn" onclick="del_user_msg(' + u_id + ')"> 删除 </div> </div> </li>';
        }
    }
    $("#chat_recent").html(html);
    obj_recent.before('</div>');
    obj_recent.remove();
    moveDeleteFn()
}
...
function update_sellers() {
    ...
    for (var u_id in seller_list) {
        if (obj_seller.parent().find("span[u_id='" + u_id + "']").size() == 0) {
            if (seller_list[u_id]['time']) {
                var time = seller_list[u_id]['time'];
            } else {
                var time = '';
            }
            if (msg_list[u_id] != '') {
                var new_msg = seller_list[u_id]['t_msg'];
            } else {
                var new_msg = '暂无新消息';
                seller_list[u_id]['new_msg'] = '暂无新消息';
            }
            obj_seller.before('<li class="information"> <div class="informlist-li"> <a href="/chat/index.php?act=web_chat&op=chat_online&goods_id=' + seller_list[u_id]['goods_id'] + '&uid=' + u_id + '"><div class="icon"> <div class="customer_touxiang"> <img src="' + seller_list[u_id]['avatar'] + '"/> </div> <div class="customerinforlist"> <p> <span class="inforlistname"> ' + seller_list[u_id]['u_name'] + ' </span> <span class="inforlistdate"> ' + time + ' </span> </p> <p> <span id="recent_t_msg' + u_id + '" class="inforlistintor"> ' + seller_list[u_id]['new_msg'] + ' </span> <span u_id="' + u_id + '" class="" style="display:none;"> 9 </span> </p> </div> </div> </a><div class="btn" onclick="del_user_msg(' + u_id + ')"> 删除 </div> </div> </li>');
        }
    }
    obj_seller.remove();
